<template>
  <div v-entersearch:Enter="{is_child:false,func:search}">
    ...
    <el-table
      ...
      style="margin:12px 24px;width:1081px;max-width:98%"
    ...
    >
      ...
      <el-table-column prop="saleAmount" width="120" label="营业额" align="center">
        <template slot="header">
          <span class="table-header">营业额
      ...
      <el-table-column prop="costAmount" width="120" label="营业成本" align="center">
        <template slot="header">
          <span class="table-header">营业额
      ...
      <el-table-column prop="grossAmount" width="120" label="营业毛利" align="center">
        <template slot="header">
          <span class="table-header">营业毛利
      ...
      <el-table-column prop="saleRagrossPercentagete" width="160" label="营业毛利占比" align="center">
        <template slot="header">
          <span class="table-header">营业毛利占比
        ...
        <template slot-scope="{row}">
          {{ DECIMAL.mul(row.grossPercentage,'100') }} %
        </template>
      </el-table-column>
      <el-table-column prop="grossRate" width="120" label="营业毛利率" align="center">
        <template slot="header">
          <span class="table-header">营业毛利率
            <el-tooltip v-if="!$store.state.app.tooltipDisabled" content="(毛利/营业额)*100%" placement="top" effect="light">
              <i class="el-icon-warning-outline" />
            </el-tooltip>
          </span>
        </template>
      ...
      <el-table-column prop="storeArea" width="120" label="门店面积" align="center" />
      <el-table-column prop="areaEfficacy" width="120" label="门店坪效" align="center">
        <template slot="header">
          <span class="table-header">门店坪效
            <el-tooltip v-if="!$store.state.app.tooltipDisabled" content="营业额/门店面积" placement="top" effect="light">
              <i class="el-icon-warning-outline" />
            </el-tooltip>
          </span>
        </template>
        <template slot-scope="{row}">
          {{ fen2yuan(row.areaEfficacy) }}
        </template>
      </el-table-column>
...
import { AreaEfficacy } from '@/api/bms/AreaEfficacy'
...
export default {
  ...
  data() {
    return {
      ...
      goodsOptions: [],
      curr_date: [dayjs().subtract(1, 'day').format('YYYY-MM-DD'), dayjs().format('YYYY-MM-DD')],
      table_loading: false,
      ids: [],
      showSummary: false,
  ...
  methods: {
    ...
    search() {
      ...
      AreaEfficacy(searchForm).then(res => {
