<template>
  <div>
    ...
    <el-table
      ...
      :style="{width:minxin_width_total(ArrFilter($attrs['form-data'],{show:true}),{op:true,opWidth:56+(mixTableOprateLength===1 && 70 || mixTableOprateLength>1 && mixTableOprateLength*40 || 0)})}"
...
import { getPage, deleteOrders, review } from '@/api/wms/vehicle'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '车辆调度单号', prop: 'vehicleSchedulOrderNo', type: 'text', width: '300px' },
        { title: '审核状态', prop: 'auditStatu', type: 'select', selectData: 'audit_status', width: '300px' },
        { title: '司机', prop: 'driver', type: 'select', defaultValue: 0, selectData: 'personListSJ', selectOption: { label: 'surname', value: 'id' }, width: '300px' },
        { title: '司机电话', prop: 'phone', type: 'text', width: '300px' },
        { title: '车牌号', prop: 'carNumber', type: 'text', width: '300px' },
        { title: '车辆类型', prop: 'carType', type: 'select', defaultValue: 0, selectData: 'CAR_TYPE', width: '300px' },
        ...
        { title: '调度日期', prop: 'schedulDate', type: 'date', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['pd:vehicle:schedul:order:query'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEd', type: 'primary', permission: ['pd:vehicle:schedul:order:delete'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['pd:vehicle:schedul:order:query'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['pd:vehicle:schedul:order:save'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-minus', disabled: 'isRowNoRevEd', type: 'primary', permission: ['pd:vehicle:schedul:order:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEd', type: 'primary', permission: ['pd:vehicle:schedul:order:verify'], width: 80 }
  ...
  methods: {
      ...
      getPage(searchForm).then(res => {
        ...
        if (res.code === 200) {
          if (res.data.records.length === 0 && searchForm.pageNum !== 1) {
            ...
            this.tableData.records.map((item, index) => { this.tableData.records[index].id = res.data.records[index].id.toString() })
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('vehicleSchedulOrderNo', this.$attrs['form-data'])}「${this.currentRow.vehicleSchedulOrderNo}」, 是否继续?`, '提示', {
