...
import { getPage, deleteOrders, review, generate, repltOrderInvalidation } from '@/api/wms/purreplenishment'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '补货单号', prop: 'replenishmentOrderNo', type: 'text', width: '300px' },
        ...
        { title: '补货日期', prop: 'purReplenishDate', type: 'date', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:pur:replenishment:order:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabledFn: this.InvalidDisFn, disabled: 'isRowNoRevEd', type: 'primary', permission: ['pur:replenishment:order:save'], width: 80 },
        { title: '失效', show: true, prop: 'Invalid', click: this.Invalid, loading: false, icon: 'el-icon-circle-close', disabledFn: this.InvalidDisFn, disabled: 'isRow', type: 'primary', permission: ['wms:pur:diff:order:repltOrderInvalidation'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:pur:replenishment:order:getList'], width: 80 },
        ...
        // { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:pur:replenishment:order:getList'], width: 80 },
        // { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEd', type: 'primary', permission: ['pur:replenishment:order:save'], width: 80 },
        ...
        // { title: '审核', show: true, prop: 'edit', click: this.$listeners.review, loading: false, icon: 'el-icon-s-check', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:replenishment:order:reviewer'], width: 80 },
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabledFn: this.reviewsDisFn, disabled: 'idsNoRevEd', type: 'primary', permission: ['wms:pur:replenishment:order:reviewer'], width: 80 }
        // { title: '生成收货单', show: true, prop: 'edit', click: this.generateFunction, loading: false, icon: 'el-icon-edit-outline', disabled: 'idsNoWillRev', disabledFn: this.isExistGenerateReceive, type: 'primary', permission: ['wms:pur:replenishment:order:generateReceiveOrder'], width: 80 }
  ...
  methods: {
    handleSelectionChange(rows) {
      ...
      this.replenishmentOrderNos = rows.map(item => item.replenishmentOrderNo)
    },
    message(res) {
      if (res.code === 200) {
    ...
    InvalidDisFn(row) {
      return row.orderInvalid || row.generateReceive
    },
    reviewsDisFn() {
      return this.mixin_rows.some(item => item.orderInvalid)
    },
    Invalid(btnItem, isSure = false) {
      ...
      repltOrderInvalidation({ repltOrderId: this.currentRow.id }).then(res => {
        this.message(res)
      }).finally(() => {
        btnItem.loading = false
      })
    ...
    isExistGenerateReceive() {
      // 存在已经生成收货单的或者失效情况
      return this.mixin_rows.some((item) => item.generateReceive || item.orderInvalid)
      // return false
    },
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('replenishmentOrderNo', this.$attrs['form-data'])}「${this.currentRow.replenishmentOrderNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    generateFunction(e, isSure = false) {
      if (!isSure) {
      ...
      e.loading = true
      generate({ replenishmentOrderNos: this.replenishmentOrderNos }).then(res => {
        this.message(res)
