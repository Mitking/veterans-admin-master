...
import { getPage, deleteOrders, review } from '@/api/wms/purrefund'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '退款单号', prop: 'refundOrderNo', type: 'text', width: '300px' },
        { title: '差异单号', prop: 'diffOrderNo', type: 'text', width: '300px' },
        ...
        { title: '采购员', prop: 'purPersonId', type: 'select', defaultValue: 0, selectData: 'personListCG', selectOption: { label: 'surname', value: 'id' }, width: '300px' },
        // { title: '供应商', prop: 'supplierId', type: 'selectFromPage', selectForm: 'SupplierIndex', width: '300px' },
        ...
        { title: '退货门店', prop: 'storeId', type: 'select', selectData: 'stores', defaultValue: 0, selectOption: { label: 'name', value: 'id' }, width: '300px' },
        { title: '退货仓库', prop: 'stockId', type: 'select', selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, width: '300px', fatherProp: 'storeId' },
        { title: '退款日期', prop: 'refundDate', type: 'date', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:pur:refund:order:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:refund:order:save'], width: 80 }

      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:pur:refund:order:getList'], width: 80 },
        ...
        // { title: '审核', show: true, prop: 'edit', click: this.$listeners.review, loading: false, icon: 'el-icon-s-check', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:refund:order:reviewer'], width: 80 },
        { title: '审核', prop: 'edit', show: true, click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEd', type: 'primary', permission: ['wms:pur:refund:order:reviewer'], width: 80 }
  ...
  methods: {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) { // 操作
      ...
      delete searchForm.supplierName
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('refundOrderNo', this.$attrs['form-data'])}「${this.currentRow.refundOrderNo}」, 是否继续?`, '提示', {
