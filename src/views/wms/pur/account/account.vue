    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          ...
          <div v-else-if="item.type==='PRICE_AUTOMATIC'">{{ fen2yuan(row[item.prop]) }}</div>
          <!--          v-else-->
...
import { getPage, deleteOrder, review } from '@/api/wms/account'
export default {
  name: 'Order',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '挂账单号', prop: 'onAccountOrderNo', type: 'text', width: '300px' },
        { title: '挂账日期', prop: 'onAccountDate', type: 'date', width: '300px' },
        { title: '审核状态', prop: 'auditStatu', defaultValue: 0, type: 'select', selectData: 'audit_status', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:pur:on:account:order:get'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:on:account:order:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:pur:on:account:order:list'], width: 80 },
        ...
        // { title: '审核', show: true, prop: 'edit', click: this.$listeners.review, loading: false, icon: 'el-icon-s-check', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:on:account:order:reviewer'], width: 80 },
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEd', type: 'primary', permission: ['wms:pur:on:account:order:reviewer'], width: 80 }
  ...
  methods: {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('applyNo', this.$attrs['form-data'])}「${this.currentRow.applyNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteOrder({ 'applyNo': this.currentRow.applyNo }).then(res => {
          this.message(res)
