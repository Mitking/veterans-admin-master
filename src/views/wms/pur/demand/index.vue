  ...
  data() {
    return {
      detail_table_buttons: [
        ...
        { title: '批量添加', show: true, prop: null, click: 'addItems', windowName: 'StoreProductIndex', formProData: { ifTotalStock: false, orgId: 0 }}
      ],
      form_bottons: [
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        // { title: '组织', width: 200, prop: 'orgId', readonly: false, show: true, type: 'select', defaultValue: this.$store.getters.orgId, selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, editShow: true, tableHidden: [1, 2].includes(this.$store.getters.accountType) ? true : undefined, detailShow: false, addFatherProp: 'Req' },
        { title: '需求门店', width: 200, prop: 'storeId', readonly: true, show: true, type: 'select', defaultValue: this.$store.state.user.storeId, selectData: 'stores', selectOption: { label: 'name', value: 'id' }, editShow: true, detailShow: true, addFatherProp: 'Req',
        ...
        // { title: '总金额', width: 150, prop: 'amount', show: true, type: 'PRICE_AUTOMATIC', design: this.mixin_auto_total_amount, arg: ['amount'], editShow: true, detailShow: true, addFatherProp: 'Req', placeholder: '系统计算' },
        ...
        { title: '商品代码', prop: 'goodsCode', click: 'addItems', windowName: 'StoreProductIndex', type: 'selectFromPage', formProData: { orgId: 0, ifTotalStock: false, stopBuy: false }, show: false, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '商品名称', prop: 'goodsName', tabWidth: 200, click: 'addItems', windowName: 'StoreProductIndex', type: 'selectFromPage', formProData: { orgId: 0, ifTotalStock: false, stopBuy: false }, show: false, editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '单位', prop: 'purUnit', tabWidth: 100, show: false, type: 'select', selectData: 'weight_type', editShow: true, detailShow: true, addFatherProp: 'detailReqs', readonly: true, defaultValue: '-' },
        ...
        { title: '单价(隐藏)', prop: 'price', realProp: 'costPrice', show: false, tableHidden: true, type: 'COST_PRICE', defaultValue: '-', readonly: true, editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        { title: '需求数量', prop: 'number', show: false, type: 'weight_number', placeholder: '手动录入', float: { decimal: 3 }, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '金额', prop: 'amount', show: false, type: 'PRICE_AUTOMATIC', design: this.mixin_auto_amount, arg: ['price', 'number'], tableHidden: true, placeholder: '系统计算', editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
  ...
  methods: {
    ...
    formSave(arg, data = { Req: {}, detailReqs: [] }, deleteDetails) { // 提交回调处理
      switch (arg.opt || arg) {
        ...
        case 'insert': // 新增
          ...
          delete data.Req
          data.amount = 0
          data.demandDetails.map(item => {
            const temp = new this.DECIMAL(item.price).mul(item.number).mul('0.001').toFixed(0)
            // item.amount = this.DECIMAL.mul(temp, '10000').toNumber()
            item.amount = temp
            data.amount = this.DECIMAL.add(data.amount, item.amount).toNumber()
          })
    ...
    deleteDetail(item) {
      return new Promise(resolve => {
        this.$confirm(`此操作将删除${this.mix_comp_find_order_title_name('demandOrderNo', this.table_data_arr)}「${item.demandOrderNo}」中的「${item.goodsName}」, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
