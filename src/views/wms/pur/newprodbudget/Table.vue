<template>
  ...
  <el-table
    ref="Tab"
    size="mini"
    style="margin:0 24px;max-width:98%;width:1020px"
  ...
  >
    ...
    >
      <template slot-scope="{row}">
        <el-input
          v-model="row.priceShow"
          v-number-input.float="{decimal:2,max:99999999}"
          placeholder="请输入"
          @input="()=>{
            row.price=yuan2fen(row.priceShow)
          }"
        />
      </template>
    </el-table-column>
    ...
    >
      <template slot-scope="{row}">
        <el-input
          ...
          v-number-input.float="{decimal:2,max:100}"
          placeholder="请输入"
        />
      </template>
    </el-table-column>
