        ...
        <template slot-scope="{row}">
          <el-form size="mini" label-position="left" inline class="demo-table-expand">
            <el-table
              ...
              style="width: 1021px;margin:10px"
            >
              <!-- <el-table-column
                label="商品代码"
                prop="goodsCode"
    ...
    >
      <div style="width: 100%;padding-right:15px">
        <component
          ...
          :no_multiple="false"
          :child_search_data="{whetherNew:1}"
...
import { newProdBudget, dataTotal, exportDataTotal } from '@/api/wms/newprod'
...
export default {
  ...
  data() {
    return {
      ...
      operateList: [
        { title: '采购计算', prop: 'search', click: this.search, icon: 'el-icon-search', permission: ['analysis:budget:newProdBudget'] },
        { title: '查看已选', prop: 'products', click: this.dialogShow, icon: 'el-icon-finished', permission: ['analysis:budget:newProdBudget'] },
        { title: '合计', prop: 'dataTotal', click: this.searchTotal, icon: 'el-icon-paperclip', permission: ['analysis:budget:dataTotal'] },
        { title: '导出', prop: 'output', click: this.exportData, icon: 'el-icon-download', permission: ['analysis:budget:newProdBudget'] },
  ...
  methods: {
    ...
    search() {
      ...
      this.$refs.searchForm.validate(pass => {
        if (pass) {
          ...
          newProdBudget(searchForm).then(res => {
    ...
    searchTotal() {
      ...
      this.tableData.map(item => {
        item.purchaseList.map(it => {
          it.lastPurPrice = it.customerUnitprice
        })
      })
    ...
    SearchDialogSelect(e, un) {
      ...
      this.purchaseList = this.MergeArray(this.purchaseList, e).filter(item => !un.map((u) => u.goodsCode).includes(item.goodsCode))
        .map(itemD => ({
          price: itemD.price,
          priceShow: this.fen2yuan(itemD.price),
          proportion: itemD.proportion || 1,
          lastPurPrice: itemD.lastPurPrice,
          goodsName: itemD.goodsName,
          goodsCode: itemD.goodsCode
        }))
    }
  }
}
