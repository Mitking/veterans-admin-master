...
import { getPage, deleteOrders, review } from '@/api/wms/purcheck'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '核验单号', prop: 'purCheckOrderNo', type: 'text', width: '300px' },
        ...
        { title: '核验日期', prop: 'purCheckDate', type: 'date', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:pur:check:order:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:check:order:save'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:pur:check:order:getList'], width: 80 },
        ...
        // { title: '审核', show: true, prop: 'edit', click: this.$listeners.review, loading: false, icon: 'el-icon-s-check', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:check:order:reviewer'], width: 80 },
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEd', type: 'primary', permission: ['wms:pur:check:order:reviewer'], width: 80 }
  ...
  methods: {
    ...
    reviews(e, isSure = false) { // 批量审核
      ...
      const unCheckArr = this.mixin_rows.some(itemF => {
        if (itemF.details.filter(item => !item.inspection).length > 0) {
          order = itemF
          return true
        }
        return false
      ...
      if (unCheckArr) {
        ...
        this.$message.error(`单号：${order.purCheckOrderNo}中存在未核验商品，请核验后重试`)
        e.loading = false
        return
      }
      if (!isSure) {
