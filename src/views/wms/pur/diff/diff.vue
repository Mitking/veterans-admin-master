...
import { getPurPage, deleteOrders, review, generateOrder } from '@/api/wms/purdiff'
...
export default {
  ...
  data() {
    return {
      ...
      operateTableList: [
        ...
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEd', type: 'primary', permission: ['wms:pur:diff:order:save'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:pur:diff:order:getPurList'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'review', click: this.reviews, loading: false, disabled: 'idsNoRevEd', icon: 'el-icon-s-check', type: 'primary', permission: ['wms:pur:diff:order:reviewer', 'wms:pur:diff:order:generateOrder'], width: 80 }
        // { title: '批量审核并生成处理单据', show: true, prop: 'reviews', click: this.reviews, loading: false, icon: 'el-icon-edit-outline', disabled: 'ids', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 }
        // { title: '相关业务', show: !this.$attrs.is_child, prop: 'otherProfession', click: this.productCopy, loading: false, icon: 'el-icon-edit-outline', disabled: false, type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80,
        //   otherProfession: [
        //     { title: '生成差异相关单', show: true, prop: 'generateDiff', click: this.generateFunction, loading: false, icon: 'el-icon-edit-outline', disabled: 'idsNoWillRev', type: 'primary', permission: ['wms:pur:diff:order:generateOrder'], width: 80 }
        //   ]
        // }
      ],
      ids: [],
  ...
  methods: {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) { // 操作
      ...
      getPurPage(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
    ...
    reviews(btnItem, isSure = false) { // 处理单据
      btnItem.loading = true
      // for (let i = 0; i < this.currentRow.details.length; i++) {
      //   if (typeof this.currentRow.details[i].billType === 'undefined' || this.currentRow.details[i].billType === '') {
      //     this.$message.error(`差异单号「${this.currentRow.details[i].purDiffOrderNo}」中「${this.currentRow.details[i].goodsName}」未选择处理方式`)
      //     btnItem.loading = false
      //     return
      //   }
      // }
      if (!isSure) {
        this.$confirm(`确认审核所选项目吗？`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      review({ 'ids': this.ids }).then(() => {
        generateOrder({ purDiffOrderNos: this.mixin_rows.map(item => item.purDiffOrderNo) }).then(res => {
          this.message(res)
        }).finally(() => {
          btnItem.loading = false
        })
