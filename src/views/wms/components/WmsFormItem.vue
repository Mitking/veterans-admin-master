  ...
  <div>
    <el-form ref="form" class="form-fa" :rules="rules" :model="form" label-width="120px">
      <el-form-item
        ...
        :label="(item.type==='remark' || item.type==='orderFrom')?'':item.title"
        :prop="item.prop"
      >
        <!--        -->
        <el-input
...
export default {
  ...
  watch: {
    ...
    '$attrs.add_arr': { // 当有添加数据时
      handler(n) {
        if (this.$attrs.select_window_name === 'form_DiffAutoSelect') { // 差异单选择
          this.form.relevantNo = this.$attrs.is_selected_arr
        }
        if (n.length > 0) {
          switch (this.$attrs.select_window_name) {
            // case 'form_DiffAutoSelect': // 差异单选择
            //   console.log(n)
            //   console.log(this.$attrs.un_select)
            //   console.log(this.$attrs.is_selected_arr)
            //   // this.form.relevantNo = this.MergeArray(this.form.relevantNo, n.map(item => item.id)).filter(itemF => !this.$attrs.un_select.map((u) => u.id).includes(itemF))
            ...
            case 'form_ProductIndex': // 商品选择
              this.form.goodsCode = n[0].goodsCode
              this.form.goodsName = n[0].goodsName
              this.form.shelfLife = n[0].shelfLife
              this.form.remindDay = n[0].remindDay
              this.form.productId = n[0].id
              this.form.barcode = n[0].barcode
              break
          }
        }
      } }
      ...
      return new Promise(resolve => {
        ...
        if (['edits', 'insert'].includes(this.$attrs['operate-item'].prop)) { // 创建,批量修改
          ArrFilter(this.$attrs['form-data'], { addFatherProp: 'Req', 'editShow': true }).map(item => {
            ...
            if (item.type === 'PRICE' || item.type === 'COST_PRICE' || item.type === 'PRICE_AUTOMATIC_SWITCH') {
              temp[item.prop + 'Show'] = ''
            }
          ...
          ArrFilter(this.$attrs['form-data'], { addFatherProp: 'Req', 'editShow': true }).map(item => {
            ...
            if (item.type === 'PRICE' || item.type === 'PRICE_AUTOMATIC_SWITCH') {
