<template>
  ...
  <keep-alive :max="2" :exclude="window_name==='DiffAutoSelect'?'WmsAddCardForm':''">
    <component
      :is="Comp"
      ref="Comp"
...
export default {
  ...
  methods: {
    ...
    openSelectWindow(e) {
      ...
      if (this.window_name === 'AlgSelect') {
        console.log(e.addedList)
        this.table_goods_code_arr = e.addedList.map(item => item.id)
        console.log(this.table_goods_code_arr)
      }
      if (e.curr.id) {
