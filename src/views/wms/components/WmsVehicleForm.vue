...
import WmsTable from '@/views/wms/components/WmsTableNodes'
...
export default {
  ...
  methods: {
    onSubmit(arg) {
      ...
      if (arg === 'detail') { // 关闭
        if (this.$attrs['operate-item'].prop === 'detail') { // 详情关闭
        ...
        this.$attrs.details.map(item => {
          deal_data.push(item)
          deal_data.push({ carType: item.notes })
        })
        if ((this.$refs.Table && JSON.stringify(this.$refs.Table.form.details) !== JSON.stringify(deal_data) || this.$refs.Table.deleteDetails.length !== 0)) {
      ...
      if (this.$refs.Table.$refs.nodeForm && this.$refs.Table.$refs.nodeForm.length > 0) {
        promise.push(...this.$refs.Table.$refs.nodeForm.map(item => item.validate()))
      }
      Promise.all(promise).then(res => {
        console.log('通过检验')
        const temp = this.deepClone(this.$refs.Form.form)
        this.$set(temp, 'deleteDetails', this.$refs.Table.deleteDetails)
        const temp_details = []
        const detailsList = this.deepClone(this.$refs.Table.form.details)
        detailsList.map((item, index) => {
          if (index % 2 === 0) { // 0,2,4
            item.nodes = []
            item.id = typeof item.id === 'undefined' ? null : item.id
            temp_details.push(item)
          } else { // 1,3,5
            this.$set(temp_details[(index - 1) / 2], 'nodes', item.carType)
            if (temp_details[(index - 1) / 2].notes) { // 删除notes
              delete temp_details[(index - 1) / 2].notes
            }
          }
        })
        this.$set(temp, 'details', temp_details)
