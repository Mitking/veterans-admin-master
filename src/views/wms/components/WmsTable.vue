<template>
  <div class="my-class">
        ...
        >
          ...
          <template slot-scope="{row,$index}">
            ...
            <el-form-item v-else :prop="`details[${$index}][${item.prop}]`" :rules="item.rule" :style="{'width':'100%','height':'30px'}">
              ...
              <!-- 费用 -->
              <el-input
                v-if="item.type==='costFees'"
                v-model="row[item.prop]"
                v-number-input.float="{max:999999,decimal:2}"
                :readonly="item.readonly"
                :placeholder="item.placeholder || ''"
                @input="()=>{row.costFees.filter(itemZ=>itemZ.feeId===item.feeId)[0].fee=yuan2fen(row[item.prop])}"
              />
              <!-- 重量 -->
              <el-input
    ...
    <el-dialog
      v-if="aoto_form_show"
      :visible.sync="aoto_form_show"
      :modal-append-to-body="false"
      :close-on-click-modal="false"
      width="500px"
    >
      <auto-form
        :arr-prop-list="ArrAutoSomeFilter( $attrs['form-data'],autoFormPropFilter)"
        :is_readonly="$attrs['operate-item'].prop==='detail' || $attrs['edit-detail-disable']"
        :form="autoFormData"
        :out_form="$attrs.form_data"
      />
      <div slot="footer">
        <el-button type="primary" @click="aoto_form_show = false">确 定</el-button>
      </div>
    </el-dialog>
    <!--  附件弹窗  -->
    <el-dialog
...
export default {
  ...
  computed: {
    ...
    costFeesDeal() { // 费用获取
      return function(row, item) {
        return this.fen2yuan(row.costFees.filter(it => it.feeId === item.feeId)[0].fee)
      }
    },
    comp_price_float() {
      ...
      handler(n) {
        if (n.length > 0) {
          ...
          if (this.$attrs.select_window_name.split('_')[0] === 'form') {
            if (this.$attrs.select_window_name === 'form_DiffAutoSelect') {
              this.tab_loading = true
              require('@/api/wms/purdiff').getListByRecordIds({ 'recordIds': n.map(i => i.id) }).then(res => {
                const arrTemp = []
                res.data.map(item => {
                  if (!this.form.details.some(i => i.goodsCode === item.goodsCode && i.price === item.receivePrice && i.diffNumQuantity === item.diffQuantity)) {
                    arrTemp.push({
                      id: null,
                      amount: item.diffAmount,
                      amountShow: this.fen2yuan(item.diffAmount),
                      price: item.receivePrice,
                      priceShow: this.fen2yuan(item.receivePrice),
                      diffNumQuantity: item.diffQuantity,
                      diffNumQuantityWeightShow: this.g2kg(item.diffQuantity),
                      goodsCode: item.goodsCode,
                      goodsName: item.goodsName,
                      img: item.img,
                      productId: item.productId,
                      purDiffOrderNo: '',
                      purUnit: item.purUnitValue,
                      remark: '',
                      spec: item.spec,
                ...
                if (arrTemp.length) {
                  ...
                  this.form.details.push(...arrTemp)
                }
              ...
              temp = n.map(item =>
                Object.assign(
                  this.deepClone(this.initFormDetailsItem),
                  {
                    ...
                    // // 最低价
                    // origianlLowestPrice: item.lowestPrice || 0,
                    // origianlLowestPriceShow: this.fen2yuan(item.lowestPrice) || 0,
                    // nowLowestPrice: item.lowestPrice || 0,
                    // nowLowestPriceShow: this.fen2yuan(item.lowestPrice) || 0,
                    // // 最高价
                    // origianlTopPrice: item.topPrice || 0,
                    // origianlTopPriceShow: this.fen2yuan(item.topPrice) || 0,
                    // nowTopPrice: item.topPrice || 0,
                    // nowTopPriceShow: this.fen2yuan(item.topPrice) || 0,
                    // 成本价
            ...
            case 'type_AlgSelect': // 算法选择
              temp = n.map(item => Object.assign(
                this.deepClone(this.initFormDetailsItem),
                {
                  ...
                  markupQuota: item.markupQuota,
                  afterPointRound: item.afterPointRound
                }))
              if (this.$attrs.curr_id) { // 替换删除
                this.deleteDetails.push(this.$attrs.curr_id)
              }
              break
          }
          temp.map(item => {
  ...
  methods: {
      ...
      return new Promise(resolve => {
        ...
        if (['edit', 'insert'].includes(this.$attrs['operate-item'].prop)) { // 创建,修改
          ...
          this.ArrFilter(this.$attrs['form-data'], { addFatherProp: 'detailReqs', 'editShow': true }).map(item => {
            ...
            if (item.type === 'costFees') { // 费用单初始化
              temp.costFees = this.form.details[0].costFees.map(costItem => ({ feeId: costItem.feeId, feeName: costItem.feeName, fee: '' }))
            }
            if (item.realProp) {
            ...
            this.form.details = this.$attrs.details.length > 0
              ? this.$attrs.details.map(itemD => {
                Object.keys(itemD).map(key => {
                  ...
                  if (key === 'costFees') { // 费用单字段
                    itemD.costFees.map(item => {
                      itemD['fee_' + item.feeId] = this.fen2yuan(item.fee)
                    })
                  }
                })
    ...
    opFunc(e, i = null, row = { id: null }) {
      ...
      if (e.click === 'purOrderSplit') { // 分单收货
        const temp = {}
        temp.selected = this.$refs.TabItem.selection
        if (temp.selected.length === 0) {
          this.$message.info('请先勾部分收货的商品')
          return
        }
        // else if(temp.selected.some(itemSed=>itemSed.)){ // 已选商品未填写必然参数
        //  return
        // }
        // 未勾选 未选取 未选择
        temp.unselected = this.$refs.TabItem.data.filter(item => !temp.selected.includes(item))
        this.$listeners.onSubmit('purOrderSplit', temp)
        return
      }
      ...
      if (temp.lossOrderType !== undefined) { // 报损类型
        temp.if24 = this.$attrs.form_data.lossOrderType === '1'
        delete temp.lossOrderType
      }
      if (temp.rejectStoreId !== undefined) { // 审请门店
        temp.storeId = this.$attrs.form_data.rejectStoreId
        delete temp.rejectStoreId
      }
      if (temp.rejectStockId !== undefined) { // 审请门店
        temp.stockId = this.$attrs.form_data.rejectStockId
        delete temp.rejectStockId
      }

      if (temp.refundStoreId !== undefined) { // 审请门店
        temp.storeId = this.$attrs.form_data.refundStoreId
        delete temp.refundStoreId
      }
      if (temp.refundStockId !== undefined) { // 审请门店
        temp.stockId = this.$attrs.form_data.refundStockId
        delete temp.refundStockId
      }
      ...
      if (temp.receiveStoreId !== undefined) { // 结算单--入库门店
