<template>
  <div>
    ...
    <mix-operate
      ...
      :back="[0, 3, 2].includes($store.getters.accountType)"
      ...
      >
        ...
        <template slot-scope="{row}">
          ...
          <el-tag
            ...
            @click="disabledFn(row)"
          >已{{ MenuGetPropName('if_enable',row[item.prop] || false) }}</el-tag>
...
import { getPage, deleteOrders, swEnableState } from '@/api/wms/retailposregister'
export default {
  name: 'Order',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      ...
      operateTableList: [
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['retail:pos:register:save'], width: 80 }
      ...
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['retail:pos:register:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['retail:pos:register:save'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'ids', type: 'primary', permission: ['retail:pos:register:delete'], width: 80 }
        // { title: '导出', show: true, prop: 'edit', click: this.output, loading: false, icon: 'el-icon-edit-outline', disabled: 'ids', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 }
  ...
  methods: {
    disabledFn(row, isSure = false) { // 禁用启用
      // e.loading = true
      if (!isSure) {
        this.$confirm(`确认${row.enableFlag ? '禁用' : '启用'}吗？`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.disabledFn(row, true)
        }).catch(() => {
      ...
      swEnableState({ 'id': row.id }).then(res => {
        this.message(res)
        // e.loading = false
      })
    },
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除「${this.$attrs['current-row'].deviceName}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
