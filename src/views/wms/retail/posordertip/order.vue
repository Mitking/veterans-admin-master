...
import { deleteOrder, getPage } from '@/api/wms/posordertip'
export default {
  name: 'Order',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '组织', prop: 'orgId', type: 'select', defaultValue: this.$store.getters.orgId, defaultAllValue: this.$store.getters.orgId, accountTypeShowFn: () => ([0, 3].includes(this.$store.getters.accountType)), selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, width: '300px' },
        { title: '门店', prop: 'storeId', type: 'select', defaultAllValue: this.$store.getters.storeId, accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: this.$store.getters.storeId, selectData: 'stores', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId', width: '300px' }
      ],
      operateTableList: [
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['oms:posOrderTip:update'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.deleteById, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['oms:posOrderTip:delete'], width: 80 }

      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:posOrderTip:getPage'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:posOrderTip:add'], width: 80 }
      ],
      ids: [],
  ...
  methods: {
    ...
    deleteById(operateItem = {}) {
      operateItem.loading = true
      this.$confirm(`此操作将永久删除「${this.currentRow.tip}」, 是否继续?`, '提示', {
