<template>
  <div>
    ...
    <el-table
      ...
      @row-click="(row) => {currentRow=row;this.$listeners.currentSelect(currentRow)}"
    >
...
import { getPage } from '@/api/oms/retailpospayconfig'
export default {
  name: 'Order',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [], // 搜索栏项目
      operateTableList: [
        // { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['oms:posPayConfig:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:posPayConfig:getList'], width: 80 }
