...
import { deleteOrder, getPage, sysncPriceTagData } from '@/api/wms/retailpricetag'
export default {
  name: 'Order',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      ruleForm: {
        storeIds: [0, 3].includes(this.$store.getters.accountType) ? [] : [this.$store.getters.storeId]
      },
      generateLoading: false,
      rules: {
        storeIds: [{ required: true, message: '请选择门店', trigger: 'blur' }]
      },
      searchList: [ // 搜索栏项目
        ...
        { title: '价签品牌', prop: 'priceTagBrand', defaultValue: 0, type: 'select', selectData: 'price_tag_brand', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['retail:store:price:tag:set:getStorePriceTagSetPage'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['retail:store:price:tag:set:updateStorePriceTagSet'], width: 80 },
        { title: '同步', show: true, prop: 'sync', click: this.syncStores, loading: false, icon: 'el-icon-upload2', disabled: 'isRow', type: 'primary', permission: ['retail:store:price:tag:set:sysncPriceTagData'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['retail:store:price:tag:set:getStorePriceTagSetPage'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['retail:store:price:tag:set:addStorePriceTagSet'], width: 80 }
      ],
      ids: [],
  ...
  methods: {
    syncStores(e, isSure = false) {
      e.loading = true
      if (!isSure) {
        this.$confirm(`确认同步价签吗？`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.syncStores(e, true)
        }).catch(() => {
      ...
      sysncPriceTagData({ storeIds: [this.currentRow.storeId] }).then(res => {
        this.message(res)
      }).finally(() => {
        e.loading = false
      })
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除编码「${this.currentRow.code}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteOrder({ 'ids': [this.currentRow.id] }).then(res => {
          this.message(res)
