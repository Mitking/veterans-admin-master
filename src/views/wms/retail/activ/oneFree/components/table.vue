<template>
  <div class="my-class">
    ...
    <el-form ref="Form" :model="form">
      <el-table ref="TabItem" :data="form.details" style="width:1211px" border>
        <el-table-column
          ...
          <template slot-scope="{row,$index}">
            ...
            <el-form-item v-else :prop="`details[${$index}].goodsCode`" :style="{'width':'100%','height':'30px'}">
              <el-input
                ...
                placeholder=""
                :readonly="true"
                @focus="insertIndex = $index;$listeners.openSelectWindow({ name: 'ProductIndex', addedList: form.details, formProData: {}, curr: row })"
              />
        ...
        <el-table-column :width="100" align="center" label="条码">
  ...
  watch: {
    ...
    '$attrs.add_arr': { // 当有添加数据时
      handler(n) {
        if (n.length > 0) {
          let temp = []
          console.log(this.$attrs.select_window_name)
