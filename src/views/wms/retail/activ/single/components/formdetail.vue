    ...
    <el-form ref="form" class="form-fa" :model="form" label-width="120px">
      ...
      <el-form-item style="width:25%" label="门店(可多选)" prop="storeIds" :rules="[{ required: true, message: '门店不能为空', trigger: 'change' }]">
        <el-tooltip
          ...
          placement="top-start"
        >
          <div slot="content" class="w50vw">{{ form.storeIds.map(item=>MenuGetPropName('stores',item,{ value: 'id', label: 'name' })).toString() }}</div>
          <div>
            {{ form.storeIds.map(item=>MenuGetPropName('stores',item,{ value: 'id', label: 'name' })).toString() | sizeFilter(20) }}
          </div>
        </el-tooltip>
      </el-form-item>
