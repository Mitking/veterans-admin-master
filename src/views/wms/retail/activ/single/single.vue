<template>
  <div v-entersearch:Enter="{is_child:false,func:search}">
    ...
    <el-form v-show="searchFormShow" ref="searchForm" class="form-fa" inline>
      <el-form-item class="item-fa" label="特价单号">
        <el-input v-model="searchForm.singleNo" placeholder="" clearable />
      </el-form-item>
    ...
    >
      ...
      />
      <!--   v-if="item.show" 表头  -->
      <el-table-column align="center" prop="singleNo" :width="240" label="特价单号" />
...
import { getPage, deleteOrders, review, OrderInvalid } from '@/api/wms/activ/single'
...
export default {
  ...
  computed: {
    ...
    invalid_comp() {
      return function(row) {
        return row.invalid ? '已作废' : row.auditStatus === '3' ? (((new Date() > new Date(row.beginTime)) && (new Date() < new Date(row.endTime))) ? '已启用' : (new Date() <= new Date(row.beginTime) && '未启用' || new Date() >= new Date(row.endTime) && '已过期')) : '-'
      }
    }
  },
