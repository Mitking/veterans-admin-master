<template>
  <div>
    <el-form ref="form" class="form-fa" :model="form" label-width="120px">
      ...
      <el-form-item style="width:25%" label="折扣率" prop="discountRate">
        <div>{{ form.discountRate }}%</div>
      </el-form-item>
      <el-form-item style="width:25%" label="价格类型" prop="priceType">
        <div>{{ MenuGetPropName('price_type',form.priceType) }}</div>
      </el-form-item>
      <!-- <el-form-item style="width:25%" label="每单限量">
        <div>{{ form.orderLimited }}</div>
      </el-form-item> -->
      ...
      <el-form-item style="width:25%" label="特价类型" prop="discountType">
        <div>{{ MenuGetPropName('discount_type',form.discountType) }}</div>
      </el-form-item>

      <!-- <el-form-item v-if="form.discountType==='2'" style="width:25%" label="商品类别" prop="categoryIds">
        <el-tooltip
          effect="dark"
          :content="form.categoryIds.map(item=>MenuGetPropName('categoryList',item,{ value: 'id', label: 'name' })).toString()"
          placement="top-start"
        >
          <div>
            {{ form.categoryIds.map(item=>MenuGetPropName('categoryList',item,{ value: 'id', label: 'name' })).toString() | sizeFilter(10) }}
          </div>
        </el-tooltip>
      </el-form-item> -->
