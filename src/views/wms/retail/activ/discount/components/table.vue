    ...
    <el-form ref="Form" :model="form">
      <el-table ref="TabItem" :data="form.details" border>
          ...
          <template slot-scope="{row,$index}">
            ...
            <el-form-item v-else :prop="`details[${$index}].goodsCode`" :style="{'width':'100%','height':'30px'}" :rules="[{ required: true, message: '不能为空', trigger: 'change' }]">
              <el-input
                ...
                placeholder=""
                :readonly="true"
                @focus="if(Number($attrs.form_data.discountRate)===0){return $message.info('请先填写折扣率')};insertIndex = $index;$listeners.openSelectWindow({ name: $store.getters.accountType===1?'StoreProductIndex':'ProductIndex', addedList: form.details, formProData: {}, curr: row })"
              />
        ...
        <el-table-column :width="100" align="center" label="零售价">
          <template slot-scope="{row}">
            <span>{{ row.salePrice?fen2yuan(row.salePrice): '-' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column :width="100" align="center" label="促销价">
          <template slot-scope="{row}">
            <span>{{ row.activPrice?fen2yuan(row.activPrice) : '系统计算' }}</span>
...
export default {
  ...
  watch: {
    ...
    '$attrs.form_data.discountRate': { // 监听表单数据
      handler(n) {
        if (n) {
          this.form.details.map(item => { item.activPrice = this.DECIMAL(n).mul('0.01').mul((this.$attrs.form_data.priceType === '5' ? item.salePrice : item.memberPrice) || '0').toNumber() })
        }
      }
    },

    '$attrs.form_data.priceType': { // 监听表单数据
      handler(n) {
        if (n) {
          this.form.details.map(item => { item.activPrice = this.DECIMAL(this.$attrs.form_data.discountRate).mul('0.01').mul((n === '5' ? item.salePrice : item.memberPrice) || '0').toNumber() })
        }
      }
    },
            ...
            case 'type_ProductIndex':
              temp = n.map(item => Object.assign(
                this.deepClone(this.initFormDetailsItem),
                {
                  ...
                  orderLimited: this.$attrs.form_data.orderLimited,
                  orderLimitedWeightShow: this.$attrs.form_data.orderLimitedWeightShow,
                  activPrice: this.DECIMAL(this.$attrs.form_data.discountRate).mul('0.01').mul((this.$attrs.form_data.priceType === '5' ? item.price : item.memberPrice) || '0').toNumber()
                }))
