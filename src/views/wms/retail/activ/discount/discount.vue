<template>
  <div v-entersearch:Enter="{is_child:false,func:search}">
    ...
    <el-form v-show="searchFormShow" ref="searchForm" class="form-fa" inline>
      <el-form-item class="item-fa" label="折扣单号">
        <el-input v-model="searchForm.discountNo" placeholder="" clearable />
      </el-form-item>
      <SelectOrgStore :type="1" @updateData="searchForm=Object.assign(searchForm,$event)" />
      <el-form-item class="item-fa" label="开始时间">
        <el-date-picker
      ...
      <el-form-item class="item-fa" label="结束时间">
        <el-date-picker
    ...
    <el-form inline size="mini" style="margin: 0 24px" :style="{'margin-top':searchFormShow?'0px':'24px','transition':transition?null:'all .3s ease'}">
      <el-form-item
        v-permission="['oms:activDiscount:getList']"
      >
        <el-button icon="el-icon-search" type="primary" @click="search(1)">查询</el-button>
      </el-form-item>
      <el-form-item
        v-permission="['oms:activDiscount:add']"
      >
      ...
      <el-form-item
        v-permission="['oms:activDiscount:update:invalid']"
      >
      ...
      <el-form-item
        v-permission="['oms:activDiscount:delete']"
      >
      ...
      <el-form-item
        v-permission="['oms:activDiscount:review']"
      >
    ...
    <el-table
      ...
      style="margin:0 24px;max-width:99%;width:1896px"
    ...
    >
      ...
      />
      <!--   v-if="item.show" 表头  -->
      <el-table-column align="center" prop="discountNo" :width="220" label="折扣单号" />
      ...
      <el-table-column align="center" prop="beginTime" :width="160" label="开始时间" sortable />
      <el-table-column align="center" prop="endTime" :width="160" label="结束时间" sortable />
      <el-table-column align="center" prop="invalid" :width="100" label="状态">
        <template slot-scope="{row}">
          {{ row.invalid?'已作废':comp_inv(row) }}
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="{ row }">
          ...
          >
            <el-button
              v-permission="['oms:activDiscountDetails:getDetails']"
              class="my-op-class"
              type="text"
              size="mini"
              :disabled="!row.id"
          ...
          >
            <el-button
              v-permission="['oms:activDiscount:update']"
              class="my-op-class"
              type="text"
              size="mini"
...
import { getPage, deleteOrders, review, OrderInvalid } from '@/api/wms/activ/discount'
...
export default {
  ...
  computed: {
    ...
    comp_inv() {
      return function(row) {
        return row.auditStatus === '3' ? (((new Date() > new Date(row.beginTime)) && (new Date() < new Date(row.endTime))) ? '已启用' : (new Date() <= new Date(row.beginTime) && '未启用' || new Date() >= new Date(row.endTime) && '已过期')) : '-'
      }
    }
  },
  watch: {
  ...
  created() {
    console.log('重新加载')
    this.TableOprateLength = this.oprate_length(['oms:activDiscountDetails:getDetails', 'oms:activDiscount:update'])
    this.search(1)
  },
  methods: {
    oprate_length(arg) {
    ...
    dels() {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrders({ 'discountNos': this.selectRows.map(item => item.discountNo) }).then(res => {
          this.message(res)
