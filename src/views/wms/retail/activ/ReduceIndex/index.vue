...
<script>
import { review, addOrder, updateOrder, getDetails } from '@/api/wms/activ/single'
import { getActivNumber } from '@/api/wms/activ/getOrderNo'
import CompForm from './components/CompForm.vue'
import Order from './reduceorder.vue'
export default {
  name: 'ReduceIndex',
  ...
  methods: {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      if (['detail', 'edit'].includes(operateItem.prop)) { // 详情，修改
        getDetails({ 'singleNo': this.currentRow.singleNo }).then(res => {
          if (res.code === 200) {
        ...
        case 'insert': // 新增
          if (data.detailReqs.length > 0) {
            data.detailReqs.map(item => {
              item.id = null
              item.singleNo = data.singleReq.singleNo
            })
          }
          if (arg.review) { // 新增并审核
