...
<script>
import { review, addOrder, updateOrder, getDetails } from '@/api/wms/activ/addreduce'
import { getActivNumber } from '@/api/wms/activ/getOrderNo'
import CompForm from './components/CompForm.vue'
import Order from './addreduce.vue'
export default {
  name: 'ActivAddReduce',
  ...
  methods: {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      if (['detail', 'edit'].includes(operateItem.prop)) { // 详情，修改
        getDetails({ 'reduceNo': this.currentRow.reduceNo }).then(res => {
          if (res.code === 200) {
        ...
        this.details = []
        getActivNumber({ type: 'HD_MJ' }).then(res => {
          if (res.code === 200) {
    ...
    formSave(arg, data = { reduceReq: {}, detailReqs: [] }) { // 提交回调处理
      console.log(data)
      switch (arg.opt || arg) {
        case 'detail': // 详情关闭
          if (arg.review) {
        ...
        case 'insert': // 新增
          if (data.detailReqs.length > 0) {
            data.detailReqs.map(item => {
              item.id = null
              item.reduceNo = data.reduceReq.reduceNo
            })
          }
          if (arg.review) { // 新增并审核
