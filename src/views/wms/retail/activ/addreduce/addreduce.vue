<template>
  <div v-entersearch:Enter="{is_child:false,func:search}">
    <!-- searchFormHeight searchFormShow -->
    <el-form v-show="searchFormShow" ref="searchForm" class="form-fa" inline>
      <el-form-item class="item-fa" label="满减单号">
        <el-input v-model="searchForm.reduceNo" placeholder="" clearable />
      </el-form-item>
    ...
    <el-form inline size="mini" style="margin: 0 24px" :style="{'margin-top':searchFormShow?'0px':'24px','transition':transition?null:'all .3s ease'}">
      ...
      <el-form-item
        v-permission="['wms:consignmentSettlement:add']"
      >
      ...
      <el-form-item
        v-permission="['oms:activReduce:update:invalid']"
      >
      ...
      <el-form-item
        v-permission="['oms:activReduce:delete']"
      >
      ...
      <el-form-item
        v-permission="['oms:activReduce:update:review']"
      >
    ...
    <el-table
      ...
      style="margin:0 24px;max-width:99%;width:1916px"
    ...
    >
      ...
      />
      <!--   v-if="item.show" 表头  -->
      <el-table-column align="center" prop="reduceNo" :width="240" label="满减单号" />
      <el-table-column align="center" prop="storeIds" :width="200" label="门店">
        <template slot-scope="{row}">
          <el-tooltip
            effect="dark"
            placement="top-start"
          >
            <div slot="content" class="w50vw">{{ row.storeIds.map(itemZ=>MenuGetPropName('stores',itemZ,{ value: 'id', label: 'name' })).toString() }}</div>
            <div>
      ...
      <el-table-column align="center" prop="beginDate" :width="160" label="开始日期" sortable />
      <el-table-column align="center" prop="endDate" :width="160" label="结束日期" sortable />

      <el-table-column align="center" prop="invalid" :width="100" label="状态">
        <template slot-scope="{row}">
          {{ invalid_comp(row) }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" prop="createTime" :width="160" label="制单日期" />
      <el-table-column align="center" prop="reviewer" :width="150" label="审核人" />
      <el-table-column align="center" prop="reviewerTime" :width="160" label="审核日期" />
      ...
      >
        <template slot-scope="{ row }">
          ...
          >
            <el-button
              v-permission="['oms:activReduceDetails:getDetails']"
              class="my-op-class"
              type="text"
              size="mini"
              :disabled="!row.id"
          ...
          >
            <el-button
              v-permission="['oms:activReduce:update']"
              class="my-op-class"
              type="text"
              size="mini"
    ...
    >
      ...
      <div slot="footer" style="marginTop:-20px">
        <el-button type="primary" @click="search_dialog_item.show = false">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import { getPage, deleteOrders, review, OrderInvalid } from '@/api/wms/activ/addreduce'
...
export default {
  ...
  computed: {
    ...
    invalid_comp() {
      return function(row) {
        return row.invalid ? '已作废' : row.auditStatus === '3' ? (((new Date() > new Date(row.beginDate)) && (new Date() < new Date(row.endDate))) ? '已启用' : (new Date() <= new Date(row.beginDate) && '未启用' || new Date() >= new Date(row.endDate) && '已过期')) : '-'
      }
    }
  },
  watch: {
  ...
  created() {
    console.log('重新加载')
    this.TableOprateLength = this.oprate_length(['oms:activReduceDetails:getDetails', 'oms:activReduce:update'])
    this.search(1)
  },
  methods: {
    oprate_length(arg) {
    ...
    dels() {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrders({ 'reduceNos': this.selectRows.map(item => item.reduceNo) }).then(res => {
          this.message(res)
