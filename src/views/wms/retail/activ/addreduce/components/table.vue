      ...
      <el-table ref="TabItem" :data="form.details" border>
        ...
        <el-table-column :width="150" align="center" label="商品代码">
          <template slot-scope="{row,$index}">
            ...
            <el-form-item v-else :prop="`details[${$index}].goodsCode`" :style="{'width':'100%','height':'30px'}">
              <el-input
        ...
        <el-table-column :width="240" align="center" label="商品名称">
          <template slot-scope="{row,$index}">
            ...
            <el-form-item v-else :prop="`details[${$index}].goodsName`" :style="{'width':'100%','height':'30px'}">
              <el-input
        ...
        <el-table-column :width="100" align="center" label="满购价">
          <template slot-scope="{row,$index}">
        ...
        <el-table-column :width="100" align="center" label="满购数量">
          <template slot-scope="{row,$index}">
            <span v-if="$attrs['operate-item'].prop==='detail'">{{ row.minNumber?g2kg(row.minNumber) : '-' }}</span>
            <el-form-item v-else :prop="`details[${$index}].minNumber`" :style="{'width':'100%','height':'30px'}">
              <el-input
                v-model="row.minNumberWeightShow"
                v-number-input.float="{decimal:3}"
                placeholder=""
                :readonly="false"
                @input="row.minNumber=row.minNumberWeightShow?kg2g(row.minNumberWeightShow):''"
              />
              <template slot="error" slot-scope="{error}">
                <span style="position: absolute;right: .5rem;font-size:0.7rem;color:#ff0000;pointer-events:none">{{ error }}</span>
              </template>
        ...
        <el-table-column :width="100" align="center" label="减免价">
          <template slot-scope="{row,$index}">
            <span v-if="$attrs['operate-item'].prop==='detail'">{{ fen2yuan(row.reducePrice) }}</span>
            <el-form-item v-else :prop="`details[${$index}].reducePrice`" :rules="[{ required: true, message: '不能为空', trigger: 'change' }]" :style="{'width':'100%','height':'30px'}">
              <el-input
                v-model="row.reducePriceShow"
                v-number-input.float="{decimal:2}"
                placeholder=""
                :readonly="false"
                @input="row.reducePrice=row.reducePriceShow?yuan2fen(row.reducePriceShow):''"
              />
              <!--   -->
              <template slot="error" slot-scope="{error}">
                <span style="position: absolute;right: .5rem;font-size:0.7rem;color:#ff0000;pointer-events:none">{{ error }}</span>
        ...
        <el-table-column :width="100" align="center" label="减免数量">
          <template slot-scope="{row,$index}">
            <span v-if="$attrs['operate-item'].prop==='detail'">{{ row.reduceNumber?g2kg(row.reduceNumber) : '-' }}</span>
            <el-form-item v-else :prop="`details[${$index}].reduceNumber`" :style="{'width':'100%','height':'30px'}">
              <el-input
                v-model="row.reduceNumberWeightShow"
                v-number-input.float="{decimal:3}"
                placeholder=""
                :readonly="false"
                @input="row.reduceNumber=row.reduceNumberWeightShow?kg2g(row.reduceNumberWeightShow):''"
              />
              <template slot="error" slot-scope="{error}">
                <span style="position: absolute;right: .5rem;font-size:0.7rem;color:#ff0000;pointer-events:none">{{ error }}</span>
              </template>
  ...
  data() {
    return {
      ...
      initFormDetailsItem: {
        ...
        'reducePriceShow': '0',
        'minNumber': 0,
        'minNumberWeightShow': '0',
        'reduceNumber': 0,
        'reduceNumberWeightShow': 0,
        'remark': ''
      },
      deleteDetails: [],
      insertIndex: null,
    ...
    init() {
      return new Promise(resolve => {
        ...
        if (['insert'].includes(this.$attrs['operate-item'].prop)) { // 创建
          ...
          console.log('修改')
          this.form.details.map(item => { this.$set(item, 'reducePriceShow', this.fen2yuan(item.reducePrice)) })
          this.form.details.map(item => { this.$set(item, 'activPriceShow', this.fen2yuan(item.activPrice)) })
          this.form.details.map(item => { this.$set(item, 'reduceNumberWeightShow', this.g2kg(item.reduceNumber)) })
          this.form.details.map(item => { this.$set(item, 'minNumberWeightShow', this.g2kg(item.minNumber)) })
        }
        resolve(true)
      })
    },
    addItems() {
      if (Number(this.$attrs.form_data.discountRate) === 0) {
        return this.$message.info('请先填写折扣率')
      }
