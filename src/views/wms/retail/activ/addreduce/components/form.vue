<template>
  <div>
    <el-form ref="form" class="form-fa" :model="form" label-width="120px">
      <el-form-item style="width:25%" label="满减单号" prop="reduceNo">
        <el-input v-model="form.reduceNo" placeholder="" :readonly="true" />
      </el-form-item>
      ...
      <el-form-item style="width:25%" label="开始时间" prop="beginDate" :rules="[{ required: true, message: '开始时间不能为空', trigger: 'change' }]">
        <el-date-picker
      ...
      <el-form-item style="width:25%" label="结束时间" prop="endDate" :rules="[{ validator: endDateCheck, trigger: 'change' }]">
        <el-date-picker
...
export default {
  ...
  methods: {
    init() {
      return new Promise(resolve => {
        ...
        if (this.$attrs['operate-item'].prop === 'insert') { // 新增
          temp.reduceNo = this.$attrs.order_no
        } else {
    ...
    endDateCheck(r, v, callback) {
      if (v === '') {
        return callback(new Error('结束时间不能为空！'))
      }

      if (v < this.form.beginDate) {
