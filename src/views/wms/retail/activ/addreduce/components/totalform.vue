<template>
  <div class="heightParent" :style="{height:(windowHeight-167)+'px'}">
    ...
    <div v-if="!$attrs.is_child" class="footer">
      ...
      <el-button v-if="$attrs['current-row'].auditStatus==='1' || $attrs['operate-item'].prop === 'insert'" v-permission="['oms:activSingle:update:review']" :loading="loading" type="primary" @click="onSubmit({opt:$attrs['operate-item'].prop,review:true})">
        <template>
          审核
        </template>
      </el-button>
  ...
  methods: {
    ...
    onSubmit(arg) {
      ...
      Promise.all(promise).then(res => {
        ...
        this.$set(temp, 'reduceReq', this.deepClone(this.$refs.Form.form))
