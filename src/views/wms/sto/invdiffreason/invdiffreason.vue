...
import { getInvDiffReasonGroup, deleteOrder, addOrder, updateOrder } from '@/api/wms/InvDiffReason'
export default {
  name: 'InvDiffReason',
  components: { },
  // 全局常用字段
  ...
  data() {
    return {
      ...
      operateTableList: [
        { title: '修改', show: true, prop: 'edit', click: this.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['wms:stoInvDiffReason:update'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['wms:stoInvDiffReason:delete'], width: 80 }

      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', permission: [], disabled: false, type: 'primary', width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoInvDiffReason:add'], width: 80 }
    ...
    message(res) {
      if (res.code === 200) {
        ...
        getInvDiffReasonGroup({ enable: true }).then(res => { // 字典表强制更新
          if (res.code === 200) {
            this.$store.dispatch('enum/add_enum', { 'diff_reason': res.data })
          }
        })
        this.$success('操作成功')
      } else {
