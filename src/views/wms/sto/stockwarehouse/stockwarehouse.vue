...
import { getPage, deleteOrder, review } from '@/api/wms/stockwarehouse'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '分拣入库单号', prop: 'sortedStorageNo', type: 'text', width: '300px' },
        { title: '商品名称', prop: 'goodsName', reallyProp: 'goodsCode', click: 'addItems', selectForm: 'ProductIndex', type: 'selectFromPage', width: '300px' },
        { title: '审核状态', prop: 'auditStatus', type: 'select', selectData: 'audit_status', width: '300px', defaultValue: 0 },
        { title: '分拣入库日期', prop: 'storageDate', type: 'date', width: '300px' },
        // { title: '入库人', prop: 'storagePersonName', reallyProp: 'storagePersonId', type: 'selectFromPage', selectForm: 'User', width: '300px' },
        { title: '入库人', prop: 'storagePersonName', reallyProp: 'storagePersonId', type: 'text', selectForm: 'User', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoWarehouseDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoWareHouse:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoWareHouse:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoWareHouse:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', disabledFn: this.isSomeAutoGenerate, type: 'primary', permission: ['wms:stoWareHouse:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', disabledFn: this.amountSomeIsZero, type: 'primary', permission: ['wms:stoWareHouse:update:review'], width: 80 }
  ...
  methods: {
    ...
    isSomeAutoGenerate() {
      // 存在上一个单的情况
      return this.mixin_rows.some((item) => !!item.storageSortedOrderNo)
    },
    amountIsZero() { // 存在0
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'sortedStorageNos': this.mixin_rows.map(item => item.sortedStorageNo) }).then(res => {
          this.message(res)
