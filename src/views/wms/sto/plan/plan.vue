<template>
  <div>
    <!--搜索栏目 @enter="transition=false;transitionMarginTop=0" deleteDetail-->
    <mix-search
    ...
    <el-table
      ...
      :style="{width:minxin_width_total(ArrFilter($attrs['form-data'],{show:true,outTabHidden:undefined,tabHidden:undefined}),{op:true,opWidth:55+(mixTableOprateLength===1 && 70 || mixTableOprateLength>1 && mixTableOprateLength*40 || 0)})}"
    ...
    >
      ...
      <!--   v-if="item.show" 表头  -->
      <el-table-column
        v-for="(item,index) in ArrFilter($attrs['form-data'],{show:true,outTabHidden:undefined,tabHidden:undefined})"
        :key="index"
        :prop="item.prop"
        align="center"
      ...
      >
        ...
        <template slot-scope="{row}">
          ...
          <div v-else-if="item.prop==='loopPeriod'">{{ row.loopTimeUnit==='2'?`每月${row.loopPeriod}号`:`每隔${row.loopPeriod}天` }}</div>
...
import { getPage, deleteOrder, review, stopInvPlan } from '@/api/wms/stockplan'
import User from '@/views/ums/admin/userSelectPage'
// import { addOrder } from '@/api/wms/stockinvtask'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '盘点计划单号', prop: 'stoInvPlanNo', type: 'text', width: '300px' },
        ...
        { title: '盘点门店', prop: 'storeId', type: 'select', accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: [0, 3, 2].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.storeId, selectData: 'stores', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId', width: '300px' },
        ...
        { title: '计划日期', prop: 'planTime', type: 'date', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoInvPlanOrder:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoInvPlanOrder:update'], width: 80 },
        { title: '停用', show: true, prop: 'disabled', click: this.enableFn, loading: false, icon: 'el-icon-circle-close', disabled: 'isRowNoWillRevs', disabledFn: (row) => row.ifStop, type: 'primary', permission: ['wms:stoInvPlanOrder:update:stop'], width: 80 }
      // disabledFn: (row) => !row.enable,
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoInvPlanOrder:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoInvPlanOrder:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoInvPlanOrder:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'review', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoInvPlanOrder:update:review'], width: 80 }
      ],
      ids: [],
  ...
  methods: {
    enableFn(e, isSure = false) {
      e.loading = true
      if (!isSure) {
        this.$confirm(`确认停用该项吗？`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.enableFn(e, true)
        }).catch(() => {
      ...
      // 停用
      stopInvPlan({ 'stoInvPlanNo': this.currentRow.stoInvPlanNo }).then((res) => {
        this.message(res)
      }).finally(() => {
        e.loading = false
      })
