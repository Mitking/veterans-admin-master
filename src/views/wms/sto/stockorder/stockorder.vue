...
import { getPage, deleteOrder, review } from '@/api/wms/stockorder'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '盘点单号', prop: 'stoInvNo', type: 'text', width: '300px' },
        ...
        { title: '盘点仓库', prop: 'stockId', type: 'select', defaultValue: 0, selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'storeId', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoInvOrderDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoInvOrder:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoInvOrder:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoInvOrder:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', disabledFn: this.isHaveStoInvTaskOrder, type: 'primary', permission: ['wms:stoInvOrder:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoInvOrder:update:review'], width: 80 }
        ...
        // { title: '相关业务', show: !this.$attrs.is_child, prop: 'otherProfession', loading: false, icon: 'el-icon-edit-outline', disabled: false, type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80,
        //   otherProfession: [
        //     { title: '生成盘点报告', show: true, prop: 'generateOrder', click: this.$listeners.generateFunction, disabledFn: this.isAutoGenerate, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoWillRevs', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 }
        //   ]
        // }
      ],
      ids: [],
  ...
  methods: {
    ...
    isHaveStoInvTaskOrder() { // 存在生成单号
      return !!this.currentRow.stoInvTaskOrder
    },
    generateFunction(e) {
      console.log(e, '接口缺失')
    },
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'stoInvNos': this.mixin_rows.map(item => item.stoInvNo) }).then(res => {
          this.message(res)
