...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        ...
        { title: '来源门店', prop: 'sourceStoreId', disabled: false, fatherProp: 'orgId', defaultValue: 0, type: 'select', selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }},
      ...
      operateTableList: [
        ...
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoReceiveOrder:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoReceiveOrder:getList'], width: 80 },
        ...
        // { title: '删除', show: !this.$attrs.is_child, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-minus', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoReceiveOrder:delete'], width: 80 },
        ...
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', disabledFn: this.amountSomeIsZero, type: 'primary', permission: ['wms:stoReceiveOrder:update:review'], width: 80 }
  ...
  methods: {
    ...
    amountSomeIsZero() { // 存在0
      return this.mixin_rows.some(item => !item.orderAmount)
    },
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('receiveOrderNo', this.$attrs['form-data'])}「${this.currentRow.receiveOrderNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteOrder({ 'receiveOrderNo': this.currentRow.receiveOrderNo }).then(res => {
          this.message(res)
