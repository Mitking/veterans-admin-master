    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          ...
          <!-- v-else -->
          <div v-else>
            <span v-if="item.prop==='purOrderNo' && !row[item.prop]" style="color:#999999">暂无关联单据</span>
...
import { getPage, deleteOrder, review } from '@/api/wms/stockreceive'
...
export default {
  ...
  data() {
    return {
      create_init: false,
      searchList: [ // 搜索栏项目
        { title: '收货单号', prop: 'receiveOrderNo', disabled: false, type: 'text', width: '300px' },
        { title: '采购单号', prop: 'purOrderNo', disabled: false, type: 'text', width: '300px' },
        ...
        { title: '收货门店', prop: 'receiveStoreId', disabled: false, accountTypeShowFn: () => false, defaultValue: this.$store.getters.storeId, type: 'select', selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }},
        { title: '收货仓库', prop: 'receiveStockId', disabled: false, type: 'select', defaultValue: 0, selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'receiveStoreId', fatherPropName: 'storeId', width: '300px' },
        { title: '收货日期', prop: 'receiveDate', disabled: false, type: 'date', width: '300px' },
        { title: '制单人', prop: 'creatorName', disabled: false, reallyProp: 'creator', type: 'selectFromPage', selectForm: 'User', width: '300px' },
        { title: '审核人', prop: 'reviewer', disabled: false, reallyProp: 'reviewerId', type: 'selectFromPage', selectForm: 'User', width: '300px' },
        { title: '供应商', prop: 'supplierName', disabled: false, placeholder: '全部', reallyProp: 'supplierId', type: 'selectFromPage', selectForm: 'SupplierIndex', width: '300px' },
        { title: '审核状态', prop: 'auditStatus', disabled: false, type: 'select', selectData: 'audit_status', defaultValue: 0, width: '300px' }
        // { title: '结算状态', prop: 'settleState', type: 'select', selectData: 'settle_state', defaultValue: 0, width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoReceiveOrderDetails:getDetails'], width: 80 },
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoReceiveOrder:update', 'wms:stoReject:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: !this.$attrs.is_child, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoReceiveOrder:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.is_child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoReceiveOrder:add'], width: 80 },
        { title: '删除', show: !this.$attrs.is_child, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', disabledFn: this.isSomeAutoGenerate, type: 'primary', permission: ['wms:stoReceiveOrder:delete', 'wms:stoReject:delete'], width: 80 },
        // { title: '审核', show: !this.$attrs.is_child, prop: 'review', click: this.$listeners.review, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', disabledFn: this.amountIsZero, type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        // { title: '反审（调试用）', show: !this.$attrs.is_child, prop: 'test', click: this.$listeners.review, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', disabledFn: this.amountSomeIsZero, type: 'primary', permission: ['wms:stoReceiveOrder:update:review', 'wms:stoReject:review'], width: 80 }
  ...
  watch: {
    ...
    '$attrs.select_window_name': {
      handler(n) {
        console.log('select_window_name:', this.$attrs.select_window_name)
        if (n === 'StockReceiveIndex' && this.$attrs.add_arr.length === 0) {
          console.log('选择窗口打开了')
          ...
          this.$set(this.searchList[4], 'disabled', true) // 收货仓库隐藏
  ...
  methods: {
    windowSelectable(row) {
      if (this.$attrs.is_child) {
        return !this.$attrs.table_goods_code_arr.includes(row.receiveOrderNo)
      }
      return true
    },
    ...
    amountIsZero() { // 存在0
      return !this.currentRow.orderAmount
    },
    amountSomeIsZero() { // 存在0
      return this.mixin_rows.some(item => (item.receiveType === '1' && !item.orderAmount))
    },
    isSomeAutoGenerate() {
      // 存在上一个单的情况
      return this.mixin_rows.some((item) => !!item.purOrderNo)
    },
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'receiveOrderNos': this.mixin_rows.map(item => item.receiveOrderNo) }).then(res => {
          this.message(res)
