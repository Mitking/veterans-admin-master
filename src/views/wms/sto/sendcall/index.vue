...
export default {
  ...
  data() {
    ...
    return {
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '申请单号', prop: 'applyNo', show: true, defaultValue: '', type: 'text', editShow: false, detailShow: true, addFatherProp: 'Req', readonly: true },
        ...
        { title: '组织', width: 200, prop: 'orgId', readonly: true, show: true, type: 'select', defaultValue: this.$store.getters.orgId, selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, editShow: true, tabHidden: true, tableHidden: true, detailShow: false, addFatherProp: 'Req' },
        { title: '发货门店', prop: 'shipStoreId', width: 200, readonly: true, show: true, type: 'select', defaultValue: this.$store.getters.storeId, selectData: 'stores', selectOption: { label: 'name', value: 'id' }, editShow: true, detailShow: true, addFatherProp: 'Req', fatherProp: 'orgId',
        ...
        { title: '收货门店', prop: 'receiptStoreId', filterable: true, placeholder: '可门店名检索', width: 200, readonly: false, show: true, type: 'select', selectData: 'stores', selectOption: { label: 'name', value: 'id' }, editShow: true, detailShow: true, addFatherProp: 'Req',
        ...
        { title: '创建人', prop: 'creatorName', width: 150, show: true, type: 'text', editShow: false, defaultValue: '暂无制单人', detailShow: true, addFatherProp: 'Req' },
        { title: '创建时间', prop: 'createTime', width: 160, show: true, type: 'text', editShow: false, defaultValue: '暂无制单人', detailShow: true, addFatherProp: 'Req' },
        ...
        { title: '商品名称', prop: 'goodsName', show: false, autoForm: ['number'], tabWidth: 200, click: 'addItems', windowName: 'StoreProductIndex', type: 'selectFromPage', formProData: { shipStoreId: '', orgId: 0 }, editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '规格', prop: 'spec', show: false, readonly: true, defaultValue: '-', click: 'addItems', windowName: 'ProductIndex', type: 'selectFromPage', editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '单价', prop: 'price', realProp: 'costPrice', show: false, type: 'COST_PRICE', autoForm: ['number'], editShow: true, readonly: false, defaultValue: '-', detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '库存数量', prop: 'totalStockNum', show: false, type: 'weight_number', autoForm: ['number'], readonly: true, editShow: true, tableHidden: true, detailShow: false, addFatherProp: 'detailReqs', placeholder: '最大限制为库存数量' },
        { title: '库存个数', prop: 'totalStockCount', placeholder: '计数商品必填', fn_disabled: (row) => !row.whetherCount, readonly: true, show: false, type: 'number', float: { decimal: 0 }, editShow: true, tableHidden: true, autoForm: ['number'], detailShow: false, addFatherProp: 'detailReqs' },
        { title: '调出个数', prop: 'count', placeholder: '计数商品必填', tableHidden: true, autoForm: ['number'], show: false, type: 'number', float: { decimal: 0 }, fn_disabled: (row) => !row.whetherCount, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '称重品', prop: 'whetherWeigh', defaultValue: '-', tableHidden: true, show: false, type: 'if_type', editShow: true, readonly: true, detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '总毛重', prop: 'weight', show: false, tableHidden: true, autoForm: ['number'], type: 'weight_number', fn_disabled: (row) => !row.whetherWeigh, editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        { title: '单件皮重(数量)', autoFormTitleFn: (row) => row.whetherWeigh ? '单件皮重' : '单件数量', prop: 'onePiecesTareWeight', show: false, tableHidden: true, autoForm: ['number'], type: 'weight_number', editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        { title: '调出件数', prop: 'callNum', show: false, type: 'number', editShow: true, tableHidden: true, autoForm: ['number'], detailShow: true, addFatherProp: 'detailReqs' },
        { title: '调出数量', prop: 'number', placeholder: '系统计算', autoForm: ['number'], focusFn: this.focusFn, autoFormPropFilter: { autoForm: 'number' }, design: this.mixin_auto_pur_num_weight, arg: ['weight', 'onePiecesTareWeight', 'callNum', 'whetherWeigh'], show: false, type: 'WEIGHT_AUTOMATIC', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ validator: numberCheck, trigger: 'change' }] },
        { title: '金额', prop: 'amount', show: false, autoForm: ['number'], type: 'PRICE_AUTOMATIC', design: this.mixin_auto_amount, autoFormPropFilter: { autoForm: 'number' }, arg: ['price', 'number'], editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
  ...
  methods: {
    ...
    sendcall_weight_member_max(row) { // 调出数量最大值函数
      return (row.totalStockNum ? row.totalStockNum / 1000 : 999999)
    },
    sendcall_count_max(row) { // 调出数量最大值函数
      return row.whetherCount ? (row.totalStockCount || 0) : 99999999
    },
    message(res) {
      if (res.code === 200) {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      // this.mixin_arr_edit_by_prop(this.table_data_arr, 'shipStoreId', 'defaultValue', this.currentRow.autoGenerate ? '' : this.$store.state.user.storeId)
      ...
      this.mixin_arr_edit_by_prop(this.table_data_arr, 'totalStockNum', 'autoForm', operateItem.prop === 'insert' ? ['number'] : [])
      this.mixin_arr_edit_by_prop(this.table_data_arr, 'totalStockCount', 'autoForm', operateItem.prop === 'insert' ? ['number'] : [])
    ...
    formSave(arg, data = { Req: {}, detailReqs: [] }) { // 提交回调处理
      switch (arg.opt || arg) {
        ...
        case 'insert': // 新增
          data.Req.id = null
          data.Req.isGenerateDelivery = true // 生成配送单
