    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          <div v-if="item.type==='if_type' || item.type==='if_enable'" :style="{color:row[item.prop]?null:'#999999'}">{{
            MenuGetPropName(item.type, row[item.prop])
          }}
          </div>
          <div v-else-if="item.type==='select' || item.type==='selectState'">{{
            MenuGetPropName(item.selectData, row[item.prop], item.selectOption ? item.selectOption : {
              value: '',
              label: ''
            })
          }}
...
import SendCallPrinter from './components/SendCallPrinter.vue'
    ...
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '调出单号', prop: 'callNo', type: 'text', width: '300px' },
        {
          title: '商品',
          prop: 'goodsName',
          reallyProp: 'goodsCode',
          click: 'addItems',
          selectForm: 'ProductIndex',
          type: 'selectFromPage',
          width: '300px'
        }, {
          title: '发货门店',
          prop: 'shipStoreId',
          defaultValue: this.$store.state.user.storeId,
          accountTypeShowFn: () => false,
          type: 'select',
          selectData: 'stores',
          width: '300px',
        ...
        },
        {
          title: '收货组织',
          prop: 'orgId',
          defaultValue: [0, 3].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.orgId,
          width: '300px',
          type: 'select',
          accountTypeShowFn: () => false,
        ...
        },
        {
          title: '审核人',
          prop: 'reviewer',
          reallyProp: 'auditPersonId',
          type: 'selectFromPage',
          selectForm: 'User',
          width: '300px'
        },
        { title: '生成验收单', prop: 'isGenerateAccept', type: 'checkbox', width: '150px' }
        // { title: '生成入库单', prop: 'isGenerateStorage', type: 'checkbox', width: '150px' }
      ],
      operateTableList: [{
        title: '详情',
        show: true,
        ...
        disabled: 'isRow',
        type: 'primary',
        permission: ['wms:sendCallDetails:getDetails'],
        width: 80
      },
      {
        title: '修改',
        ...
        disabled: 'isRowNoRevEds',
        type: 'primary',
        permission: ['wms:sendCall:update'],
        width: 80
      }],
      operateList: [ // 操作栏项目
        {
          ...
          disabled: false,
          type: 'primary',
          permission: ['wms:sendCall:getList'],
          width: 80
        },
        {
          title: '新增',
        ...
        },
        {
          ...
          loading: false,
          icon: 'el-icon-minus',
          disabled: 'idsNoRevEds',
          disabledFn: this.isAutoGenerate,
          type: 'primary',
          permission: ['wms:sendCall:delete'],
          width: 80
        },
        // { title: '审核', show: true, prop: 'edit', click: this.review, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        {
          ...
          disabled: 'idsNoRevEds',
          type: 'primary',
          permission: ['wms:sendCall:update:review'],
          width: 80
        },
        {
          title: '打印',
