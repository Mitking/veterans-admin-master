...
import { getPage, deleteOrder, review } from '@/api/wms/stockreport'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '盘点报告单号', prop: 'stoInvReportNo', type: 'text', width: '300px' },
        { title: '组织', accountTypeShowFn: () => ([0, 3].includes(this.$store.getters.accountType)), defaultValue: [0, 3].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.orgId, prop: 'orgId', type: 'select', selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, width: '300px' },
        { title: '门店', accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: [0, 3, 2].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.storeId, prop: 'storeId', type: 'select', selectData: 'stores', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId', width: '300px' },
        { title: '仓库', defaultValue: 0, prop: 'stockId', type: 'select', selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'storeId', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoInvReportDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoInvReport:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoInvReport:getList'], width: 80 },
        ...
        // { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-minus', disabled: 'isRowNoRevEds', type: 'primary', permission: ['ums:role:page'], width: 80 },
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoInvReport:update:review'], width: 80 },
        { title: '原因', show: true, prop: 'edit', click: this.$listeners.tipAddShow, loading: false, icon: 'el-icon-edit-outline', type: 'primary', permission: [], width: 80 }
  ...
  methods: {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('stoInvReportNo', this.$attrs['form-data'])}「${this.currentRow.stoInvReportNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteOrder({ 'stoInvReportNo': this.currentRow.stoInvReportNo }).then(res => {
          this.message(res)
