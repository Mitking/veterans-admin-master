...
import { getDetails, addOrder, updateOrder, deleteDetails, getStoNumber, review } from '@/api/wms/stocklossorder'
import WmsCompForm from '@/views/wms/components/WmsCompForm'
import Order from './stocklossorder'
export default {
  name: 'StockLossOrderIndex',
  ...
  data() {
    ...
    return {
      ...
      form_bottons: [
        { title: '审核', mix_comp_buttons_show: this.mix_comp_buttons_show, editShowArgs: { auditStatus: '1' }, detailShowArgs: { auditStatus: '1' }, prop: 'review', loading: false, icon: 'el-icon-s-check', type: 'primary', permission: ['wms:stoLossOrder:update:review'] }
      ],
      loading: false,
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '报损单号', prop: 'stoLossNo', show: true, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req', readonly: true },
        { title: '报损日期', prop: 'lossDate', width: 150, show: true, defaultValue: dayjs().format('YYYY-MM-DD'), type: 'date', editShow: true, detailShow: true, addFatherProp: 'Req',
        ...
        { title: '报损门店', prop: 'storeId', readonly: [1].includes(this.$store.getters.accountType), width: 200, show: true, type: 'select', defaultValue: this.$store.getters.storeId, selectData: 'stores', selectOption: { label: 'name', value: 'id' }, editShow: true, detailShow: true, addFatherProp: 'Req', fatherProp: 'orgId',
          rule: [{ required: true, message: '门店不能为空', trigger: 'change' }] },
        { title: '报损仓库', prop: 'stockId', width: 200, show: true, type: 'select', selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, editShow: true, detailShow: true, addFatherProp: 'Req', fatherProp: 'storeId',
          rule: [{ required: true, message: '仓库不能为空', trigger: 'change' }] },
        { title: '报损类型', prop: 'lossOrderType', formItemWidth: 140, width: 150, show: true, type: 'select', selectData: 'loss_order_type', editShow: true, detailShow: true, addFatherProp: 'Req',
        ...
        // { title: '报损门店', prop: 'store', show: true, type: 'text', editShow: false, detailShow: true, addFatherProp: 'Req' },
        // { title: '报损仓库', prop: 'stock', show: true, type: 'text', editShow: false, detailShow: true, addFatherProp: 'Req' },
        ...
        { title: '报损单号', prop: 'stoLossNo', show: false, type: 'text', editShow: true, detailShow: true, addFatherProp: 'detailReqs', tableHidden: true },
        { title: '商品代码', prop: 'goodsCode', show: false, click: 'addItems', windowName: 'StoreProductIndex', formProData: { storeId: '', stockId: '', orgId: 0, lossOrderType: '' }, type: 'selectFromPage', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '商品名称', prop: 'goodsName', show: false, tabWidth: 200, click: 'addItems', windowName: 'StoreProductIndex', formProData: { storeId: '', stockId: '', orgId: 0, lossOrderType: '' }, type: 'selectFromPage', editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '单价', prop: 'price', realProp: 'costPrice', show: false, type: 'COST_PRICE', defaultValue: '-', readonly: true, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '报损数量', prop: 'number', show: false, type: 'weight_number', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '报损金额', prop: 'amount', show: false, type: 'PRICE_AUTOMATIC', design: this.mixin_auto_amount, arg: ['price', 'number'], editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '报损个数', prop: 'count', show: false, placeholder: '计数品必填', type: 'number', float: { decimal: 0 }, disabledFn: (row) => !row.whetherCount, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ validator: countCheck, trigger: 'change' }] },
        { title: '原因', prop: 'lossReason', show: false, type: 'select', selectData: 'loss_reason', selectValueType: 'string', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
  ...
  methods: {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      this.mixin_arr_edit_by_prop(this.table_data_arr, 'lossOrderType', 'readonly', (operateItem.prop === 'edit' && this.currentRow.autoGenerate))
      ...
      this.mixin_arr_edit_by_prop(this.table_data_arr, 'lossReason', 'readonly', (operateItem.prop === 'edit' && this.currentRow.autoGenerate))
      this.operateItem = operateItem
      operateItem.loading = true
      if (['detail', 'edit'].includes(operateItem.prop)) { // 详情，修改
        getDetails({ 'stoLossNo': this.currentRow.stoLossNo }).then(res => {
          if (res.code === 200) {
        ...
        this.details = []
        getStoNumber({ stoType: 'CC_BS' }).then(res => {
          if (res.code === 200) {
        ...
        case 'insert': // 新增
          ...
          if (data.detailReqs.length > 0) {
            data.detailReqs.map(item => {
              item.id = null
              item.stoLossNo = data.Req.stoLossNo
            })
          }
          data.lossOrderReq = data.Req
          delete data.Req
          if (arg.review) { // 新增并审核
    ...
    deleteDetail(item) {
      return new Promise(resolve => {
        this.$confirm(`此操作将永久删除单号「${item.stoLossNo}」中的「${item.goodsName}」, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
