<template>
  <div>
    ...
    > -->
    <mix-search v-show="searchFormShow" ref="searchForm" :form="searchForm" :formlist="searchList" v-bind="$attrs" @search="search" @opForm="MixinOpenForm" @clickClear="searchForm[$event]=''" />
    <!-- </transition> -->
    <!-- 操作栏目 -->
    <mix-operate
...
import { getPage, deleteOrder, review } from '@/api/wms/stockcomboorder'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '拆分组合单号', prop: 'stoComboNo', type: 'text', width: '300px' },
        { title: '门店', prop: 'storeId', type: 'select', accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: this.$store.getters.storeId, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }},
        ...
        { title: '审核状态', prop: 'auditStatus', type: 'select', selectData: 'audit_status', defaultValue: 0, width: '300px' },
        { title: '拆分类型', prop: 'comboType', type: 'select', selectData: 'combo_type', width: '300px', defaultValue: 0 },
        { title: '拆分日期', prop: 'comboDate', type: 'date', width: '300px' },
        // { title: '操作人', prop: 'operatorName', reallyProp: 'operator', type: 'selectFromPage', selectForm: 'User', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoComboOrderDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoComboOrder:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoComboOrder:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoComboOrder:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoComboOrder:delete'], width: 80 },
        // { title: '审核', show: true, prop: 'edit', click: this.$listeners.review, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', disabledFn: this.amountIsZero, type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', disabledFn: this.amountSomeIsZero, type: 'primary', permission: ['wms:stoComboOrder:update:review'], width: 80 }
  ...
  methods: {
      ...
      getPage(searchForm).then(res => {
        ...
        if (res.code === 200) {
          if (res.data.records.length === 0 && searchForm.pageNum !== 1) {
            ...
            res.data.records.map(item => { item.costPrice = item.price })
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'stoComboNos': this.mixin_rows.map(item => item.stoComboNo) }).then(res => {
          this.message(res)
