...
import { getPage, deleteOrder, review } from '@/api/wms/senddelivery'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '配送单号', prop: 'deliveryNo', type: 'text', width: '300px', disabled: false },
        { title: '商品', prop: 'goodsName', reallyProp: 'goodsCode', disabled: false, click: 'addItems', selectForm: 'ProductIndex', type: 'selectFromPage', width: '300px' },
        { title: '司机', prop: 'driverName', reallyProp: 'driverId', disabled: false, type: 'selectFromPage', selectForm: 'User', width: '300px' },
        { title: '车牌号', prop: 'plateNumber', type: 'text', disabled: false, width: '300px' },
        { title: '发货组织', prop: 'orgId', defaultValue: [0, 3].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.orgId, width: '300px', type: 'select', accountTypeShowFn: () => ([0, 3].includes(this.$store.getters.accountType)), selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
        { title: '收货组织', prop: 'receiveOrgId', disabled: false, defaultValue: 0, type: 'select', selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, width: '300px' },
        { title: '收货门店', prop: 'receiptStoreId', type: 'select', disabled: false, defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherPropName: 'orgId', fatherProp: 'receiveOrgId' },
        { title: '收货仓库', prop: 'receiptStockId', type: 'select', disabled: false, defaultValue: 0, selectData: 'stockId', width: '300px', selectOption: { label: 'stockName', value: 'id' }, fatherPropName: 'storeId', fatherProp: 'receiptStoreId' },
        { title: '审核状态', defaultValue: 0, prop: 'auditStatus', type: 'select', disabled: false, selectData: 'audit_status', width: '300px' },
        { title: '审核人', prop: 'reviewer', reallyProp: 'reviewerId', disabled: false, type: 'selectFromPage', selectForm: 'User', width: '300px' },
        { title: '送货日期', prop: 'deliveryDate', type: 'date', disabled: false, width: '300px' },
        { title: '结算状态', prop: 'settleState', type: 'select', selectData: 'settle_state', defaultValue: 0, width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:sendDeliveryDetails:getDetails'], width: 80 },
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:sendDelivery:update'], width: 80 },
        { title: '费用管理', show: true, prop: 'fee', click: this.$listeners.feeManage, loading: false, icon: 'el-icon-notebook-1', disabled: 'isRowNoWillRevs', type: 'primary', permission: ['wms:sendDeliveryFee:getList'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: !this.$attrs.is_child, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:sendDelivery:getList'], width: 80 },
        ...
        // { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.review, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:sendDelivery:update:review'], width: 80 }
  ...
  watch: {
    ...
    '$attrs.select_window_name': {
      handler(n) {
        if (n === 'SendDeliveryIndex' && this.$attrs.add_arr.length === 0) {
          console.log('选择窗口打开了')
          ...
          this.$set(this.searchList[4], 'disabled', true) // 发货组织隐藏
          this.$set(this.searchList[7], 'disabled', true) // 收货仓库隐藏
          delete this.$attrs.child_search_data.orgId
          this.searchForm.orgId = this.$store.getters.orgId
  ...
  methods: {
    windowSelectable(row) {
      if (this.$attrs.is_child) {
        return !this.$attrs.table_goods_code_arr.includes(row.deliveryNo)
      }
      return true
    },
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('deliveryNo', this.$attrs['form-data'])}「${this.currentRow.deliveryNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteOrder({ 'deliveryNo': this.currentRow.deliveryNo }).then(res => {
          this.message(res)
