...
import { getPage, deleteOrder, review, generateWareBySorted } from '@/api/wms/stocksorted'
...
export default {
  ...
  data() {
    return {
      ...
      auditStatus: [],
      searchList: [ // 搜索栏项目
        { title: '分拣单号', prop: 'storageSortedOrderNo', type: 'text', width: '300px' },
        { title: '分拣商品', prop: 'goodsName', reallyProp: 'goodsCode', click: 'addItems', selectForm: 'ProductIndex', type: 'selectFromPage', width: '300px' },
        { title: '审核状态', prop: 'auditStatus', type: 'select', selectData: 'audit_status', width: '300px', defaultValue: 0 },
        { title: '分拣日期', prop: 'sortedDate', type: 'date', width: '300px' },
        // { title: '分拣人', prop: 'sortedPerson', reallyProp: 'sortedPerson', type: 'selectFromPage', selectForm: 'User', width: '300px' },
        { title: '分拣人', prop: 'sortedPerson', reallyProp: 'sortedPerson', type: 'text', selectForm: 'User', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoSortedOrderDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoSortedOrder:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoSortedOrder:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoSortedOrder:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoSortedOrder:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoSortedOrder:update:review'], width: 80 },
        { title: '生成分拣入库单', show: true, prop: 'generateWarehouse', click: this.generateFunction, loading: false, icon: 'el-icon-edit-outline', disabled: 'idsNoWillRevs', disabledFn: this.amountSomeIsG, type: 'primary', permission: ['wms:stoSortedOrder:generateWareBySorted'], width: 80 }
  ...
  methods: {
    amountSomeIsG() { // 存在
      return this.mixin_rows.some(item => item.isGenerateWarehouse)
    },
    generateFunction(e, isSure = false) {
      ...
      e.loading = true
      // /sto/sorted/order/generateWareBySorted
      generateWareBySorted({ 'sortedOrderNos': this.Nos }).then(res => {
        this.message(res)
      }).finally(() => {
        e.loading = false
      })
    ...
    handleSelectionChange(rows) {
      ...
      this.auditStatus = rows.map(item => item.auditStatus)
      this.Nos = rows.map(item => item.storageSortedOrderNo)
    },
    message(res) {
      if (res.code === 200) {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'storageSortedOrderNos': this.mixin_rows.map(item => item.storageSortedOrderNo) }).then(res => {
          this.message(res)
