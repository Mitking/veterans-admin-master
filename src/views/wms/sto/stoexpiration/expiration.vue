...
import { getPage, deleteOrder } from '@/api/wms/expiration'
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        ...
        { title: '执行状态', prop: 'executionState', defaultValue: 0, type: 'select', selectData: 'if_type', width: '300px' },
      ...
      operateTableList: [
        { title: '删除', show: true, prop: 'edit', click: this.dels, loading: false, icon: 'el-icon-delete', disabled: 'isRowNoRevEds', type: 'primary', permission: ['expiration:remind:delete'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['expiration:remind:expirationRemindList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['expiration:remind:add'], width: 80 }
      ],
      ids: [],
