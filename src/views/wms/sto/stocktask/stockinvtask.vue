...
import { getPage, deleteOrder, review, generateInvOrderByTask } from '@/api/wms/stockinvtask'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '盘点任务单号', prop: 'stoInvTaskNo', type: 'text', width: '300px' },
        ...
        { title: '门店', prop: 'storeId', width: '300px', type: 'select', accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: [0, 3, 2].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.storeId, selectData: 'stores', fatherProp: 'orgId', selectOption: { label: 'name', value: 'id' }},
        { title: '仓库', prop: 'stockId', width: '300px', type: 'select', defaultValue: 0, selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'storeId' },
        { title: '审核状态', prop: 'auditStatus', type: 'select', defaultValue: 0, selectData: 'audit_status', width: '300px' },
        { title: '盘点日期', prop: 'invTime', type: 'date', width: '300px' },
        ...
        { title: '只看已经生成盘点单', prop: 'isGenerateInv', type: 'checkbox', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:stoInvTaskDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:stoInvTask:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:stoInvTask:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:stoInvTask:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoInvTask:delete'], width: 80 },
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:stoInvTask:update:review'], width: 80 },
        { title: '生成盘点单', show: true, prop: 'generateOrder', click: this.generateFunction, loading: false, icon: 'el-icon-edit-outline', disabled: 'idsNoWillRevs', disabledFn: this.amountSomeIsG, type: 'primary', permission: ['wms:stoInvTask:generateInvOrderByTask'], width: 80 }
  ...
  methods: {
    amountSomeIsG() { // 存在
      return this.mixin_rows.some(item => item.isGenerateInv)
    },
    handleSelectionChange(rows) {
      ...
      this.NoList = rows.map(item => item.stoInvTaskNo)
    },
    generateFunction(e, isSure = false) {
      ...
      e.loading = true
      generateInvOrderByTask({ invTaskNoList: this.NoList }).then(res => {
        this.message(res)
      }).finally(() => {
        e.loading = false
      })
        ...
        if (res.code === 200) {
          if (res.data.records.length === 0 && searchForm.pageNum !== 1) {
            ...
            res.data.records.map(item => {
              item.invRange = item.invRange.toString()
            })
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'stoInvTaskNos': this.mixin_rows.map(item => item.stoInvTaskNo) }).then(res => {
          this.message(res)
