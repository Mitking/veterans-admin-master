...
import { getPage, deleteOrder, review } from '@/api/wms/sendreject'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      create_init: false,
      searchList: [ // 搜索栏项目
        { title: '退货单号', prop: 'rejectNo', type: 'text', width: '300px' },
        ...
        { title: '退货门店', prop: 'rejectStoreId', type: 'select', defaultValue: this.$store.getters.accountType === 1 ? this.$store.state.user.storeId : 0, accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId' },
        { title: '发货门店', prop: 'deliveryStoreId', type: 'select', disabled: !!this.$attrs.is_child, defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }},
        { title: '发货仓库', prop: 'deliveryStockId', type: 'select', defaultValue: 0, selectData: 'stockId', width: '300px', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'deliveryStoreId', fatherPropName: 'storeId' },
        { title: '退货日期', prop: 'rejectDate', type: 'date', width: '300px' },
        { title: '审核人', prop: 'reviewer', reallyProp: 'auditPersonId', type: 'selectFromPage', selectForm: 'User', width: '300px' }
        // { title: '对方是否已入库', prop: 'isGenerateStorage', type: 'checkbox', width: '150px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:sendRejectDetails:getDetails'], width: 80 },
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:sendReject:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: !this.$attrs.is_child, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:sendReject:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.is_child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:sendReject:add'], width: 80 },
        // { title: '删除', show: !this.$attrs.is_child, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-minus', disabled: 'isRow', type: 'primary', permission: ['ums:role:page'], width: 80 },
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:sendReject:update:review'], width: 80 }
      ],
      ids: [],
      ...
      otherOffsetHeight: 0,
      searchForm: {
        deliveryStoreId: this.$attrs.is_child ? this.$store.state.user.storeId : 0,
        pageNum: 1,
        pageSize: 10
      },
  ...
  watch: {
    ...
    '$attrs.select_window_name': {
      handler(n) {
        if (n === 'SendRejectIndex' && this.$attrs.add_arr.length === 0) {
          console.log('选择窗口打开了')
          ...
          this.searchForm.deliveryStoreId = this.$store.state.user.storeId
          this.searchForm.org = this.$attrs.child_search_data.receiveOrgId
  ...
  methods: {
    windowSelectable(row) {
      if (this.$attrs.is_child) {
        return !this.$attrs.table_goods_code_arr.includes(row.rejectNo)
      }
      return true
    },
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('rejectNo', this.$attrs['form-data'])}「${this.currentRow.rejectNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteOrder({ 'rejectNo': this.currentRow.rejectNo }).then(res => {
          this.message(res)
