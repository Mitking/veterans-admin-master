<template>
  <div>
    ...
    >
      <mix-search v-show="searchFormShow" :form="searchForm" :formlist="searchList" />
    </transition>
    <!-- 操作栏目 -->
    <mix-operate
...
import { getPage, deleteOrder, generate } from '@/api/wms/sendreject'
export default {
  name: 'Order',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        ...
        { title: '退货门店id', prop: 'rejectStoreId', type: 'select', defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherProp: 'companyId' },
        { title: '发货门店id', prop: 'deliveryStoreId', type: 'select', defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherProp: 'companyId' },
        { title: '发货仓库id', prop: 'deliveryStockId', type: 'select', defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherProp: 'companyId' },
      ...
      operateList: [ // 操作栏项目
        ...
        { title: '生成入库单', show: true, prop: 'edit', click: this.generates, loading: false, icon: 'el-icon-edit-outline', disabled: 'ids', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 }
  ...
  methods: {
    handleSelectionChange(rows) {
      ...
      this.rejectNos = rows.map(item => item.rejectNo)
    },
    message(res) {
      if (res.code === 200) {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) { // 操作
      ...
      searchForm.isGenerateStorage = false
    ...
    generates() { // 批量审核
      generate({ 'rejectNoList': this.rejectNos }).then(res => {
        this.message(res)
      })
    },
    transitionBegin(el) {
