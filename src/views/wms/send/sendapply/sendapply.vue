...
import { getPage, deleteOrder, review } from '@/api/wms/sendapply'
...
export default {
  ...
  mixins: [MixinGlobal],
  data() {
    const storeLevel = function() {
      const Tstore = this.$store.state.enum.enumList.stores.filter(item => item.id === this.$store.state.user.storeId)[0]
      return Tstore.storeLevel
    }
    const Tstore = () => {
    ...
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '单据号', prop: 'applyNo', type: 'text', width: '300px' },
        ...
        { title: '申请组织', prop: 'orgId', defaultValue: [0, 3].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.orgId, width: '300px', type: 'select', accountTypeShowFn: () => ([0, 3].includes(this.$store.getters.accountType)), selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
        { title: '申请门店', prop: 'applyStoreId', defaultValue: Tstore(), width: '300px', accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), type: 'select', selectData: 'stores', fatherProp: 'orgId', selectOption: { label: 'name', value: 'id' }},
        { title: '发货门店', prop: 'sendStoreId', type: 'select', defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }},
        { title: '申请日期', prop: 'applyDate', type: 'date', width: '300px' },
        { title: '审核人', prop: 'reviewer', reallyProp: 'auditPersonId', type: 'selectFromPage', selectForm: 'User', width: '300px' },
        { title: '只看生成调出单', prop: 'isGenerateCall', type: 'checkbox', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:sendApplyItem:getDetail'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:sendApply:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:sendApply:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:sendApply:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:sendApply:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:sendApply:update:review'], width: 80 }
        ...
        // { title: '相关业务，在调拨中心中操作', show: !this.$attrs.is_child, prop: 'otherProfession', loading: false, icon: 'el-icon-edit-outline', disabled: false, type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80,
        //   otherProfession: [
        //     { title: '生成调出单', show: true, prop: 'generate', click: this.generateFunction, loading: false, icon: 'el-icon-edit-outline', disabled: 'ids', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 }
        //   ]
        // }
      ],
      ids: [],
  ...
  methods: {
    // generateFunction(items, itemsF) {
    //   itemsF.loading = true
    //   generate({ 'applyNoList': this.Nos, isGenerateDelivery: this.storeLevel }).then(res => {
    //     this.message(res)
    //   }).finally(() => {
    //     itemsF.loading = false
    //   })
    // },
    handleSelectionChange(rows) {
      ...
      this.Nos = rows.map(item => item.applyNo)
    },
    message(res) {
      if (res.code === 200) {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'applyNos': this.mixin_rows.map(item => item.applyNo) }).then(res => {
          this.message(res)
