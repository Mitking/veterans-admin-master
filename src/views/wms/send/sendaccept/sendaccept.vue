...
import { getPage, deleteOrder, review } from '@/api/wms/sendaccept'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '验收单号', prop: 'acceptNo', type: 'text', width: '300px' },
        { title: '商品代码', prop: 'goodsName', reallyProp: 'goodsCode', click: 'addItems', selectForm: 'ProductIndex', type: 'selectFromPage', width: '300px' },
        ...
        { title: '收货门店', prop: 'receiptStoreId', type: 'select', defaultValue: this.$store.getters.accountType === 1 ? this.$store.state.user.storeId : 0, accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId' },
        { title: '发货门店', prop: 'shipStoreId', type: 'select', defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }},
        { title: '验收日期', prop: 'acceptDate', type: 'date', width: '300px' },
        { title: '审核人', prop: 'reviewer', reallyProp: 'auditPersonId', type: 'selectFromPage', selectForm: 'User', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:sendAcceptDetails:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:sendAccept:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:sendAccept:getList'], width: 80 },
        // { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:sendAccept:add'], width: 80 },
        // { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-minus', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:sendAccept:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:sendAccept:update:reviewAccept'], width: 80 }
      ...
      searchForm: {
        pageNum: 1,
        pageSize: 10
        // receiptStoreId: this.$store.state.user.storeId
      },
  ...
  methods: {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('acceptNo', this.$attrs['form-data'])}「${this.currentRow.acceptNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteOrder({ 'acceptNo': this.currentRow.acceptNo }).then(res => {
          this.message(res)
