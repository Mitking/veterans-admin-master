...
import { getPage, deleteOrder, review } from '@/api/wms/sendstorage'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '收货单号', prop: 'storageNo', type: 'text', width: '300px' },
        ...
        { title: '收货门店', prop: 'storageStoreId', type: 'select', defaultValue: this.$store.getters.accountType === 1 ? this.$store.state.user.storeId : 0, accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId' },
        { title: '收货仓库', prop: 'storageStockId', type: 'select', defaultValue: 0, selectData: 'stockId', width: '300px', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'storageStoreId', fatherPropName: 'storeId' },
        { title: '收货日期', prop: 'applyDate', type: 'date', width: '300px' },
        { title: '审核人', prop: 'reviewer', reallyProp: 'auditPersonId', type: 'selectFromPage', selectForm: 'User', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:sendStorageDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['wms:sendStorage:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:sendStorage:getList'], width: 80 },
        // { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:sendStorage:add'], width: 80 },
        // { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', disabledFn: this.isAutoGenerate, type: 'primary', permission: ['wms:sendStorage:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['wms:sendStorage:update:review'], width: 80 }
  ...
  methods: {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'storageNos': this.mixin_rows.map(item => item.storageNo) }).then(res => {
          this.message(res)
