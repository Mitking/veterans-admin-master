  ...
  <div>
    ...
    >
      <mix-search
        ...
        v-bind="$attrs"
        @search="search"
        @opForm="Comp=$event.selectForm;selectFormShow=true"
    ...
    <el-table
      ...
      @row-dblclick="()=>{
        if($attrs.is_child && windowSelectable(currentRow)){
          windowSelect([currentRow])
          $listeners.addlist()
        }}"
    >
      <!--   可选  -->
    ...
    >
      <div style="width: 100%;padding-right:15px">
        <component
          ...
          :no_multiple="true"
          @selectList="searchForm.goodsCode=$event[0].goodsCode;searchForm.goodsName=$event[0].goodsName;"
          @addlist="selectFormShow=false"
        />
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '配送单号', prop: 'deliveryNo', type: 'text', width: '300px' },
        { title: '商品代码', prop: 'goodsName', reallyProp: 'goodsCode', click: 'addItems', selectForm: 'ProductIndex', type: 'selectFromPage', width: '300px' },
        { title: '司机id', prop: 'driverId', type: 'text', width: '300px' },
        { title: '车牌号', prop: 'plateNumber', type: 'text', width: '300px' },
        { title: '收货门店id', prop: 'receiptStoreId', type: 'select', defaultValue: 0, selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }},
        { title: '收货仓库id', prop: 'receiptStockId', type: 'select', defaultValue: 0, selectData: 'stockId', width: '300px', selectOption: { label: 'stockName', value: 'id' }, fatherPropName: 'storeId', fatherProp: 'receiptStoreId' },
        { title: '审核状态', defaultValue: 0, prop: 'auditStatus', type: 'select', selectData: 'audit_status', width: '300px' },
        { title: '审核人id', prop: 'auditPersonId', type: 'text', width: '300px' },
        { title: '送货日期', prop: 'deliveryDate', type: 'date', width: '300px' }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['ums:role:page', 'ums:role:page'], width: 80 },
        ...
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        ...
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.review, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-edit-outline', disabled: 'ids', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 }
