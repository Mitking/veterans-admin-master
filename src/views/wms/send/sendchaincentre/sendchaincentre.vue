...
import { getPage, batchDeal } from '@/api/wms/sendapply'
import { getSendCallPage } from '@/api/wms/sendcall'
...
export default {
  ...
  data() {
    return {
      ...
      operateList: [ // 操作栏项目
        ...
        { title: '生成相关单据', show: true, prop: 'deal', click: this.dealOrders, loading: false, icon: 'el-icon-edit-outline', disabled: 'ids', type: 'primary', permission: ['wms:sendCenter:deal'], width: 80 }
      ]
    }
  },
  computed: {},
  ...
  methods: {
    search() {
      ...
      const P1 = { auditStatus: '3', applyNo: searchForm.orderNo, isGenerateCall: false, pageNum: 1, pageSize: 100, applyDate: searchForm.orderDate, applyStoreId: searchForm.storeId, sendStoreId: this.$store.state.user.storeId, orgId: this.$store.getters.orgId }
      const P2 = { auditStatus: '3', callNo: searchForm.orderNo, isGenerateAccept: false, pageNum: 1, pageSize: 100, callDate: searchForm.orderDate, shipStoreId: searchForm.storeId, receiptStoreId: this.$store.state.user.storeId, orgId: this.$store.getters.orgId }
      const P3 = { auditStatus: '3', rejectNo: searchForm.orderNo, isGenerateStorage: false, pageNum: 1, pageSize: 100, rejectDate: searchForm.orderDate, rejectStoreId: searchForm.storeId, deliveryStoreId: this.$store.state.user.storeId, orgId: this.$store.getters.orgId }
      this.getPageList = [
    ...
    dealOrders(e, isSure = false) { // 处理单据
      ...
      // const pArr = [
      //   this.applyNos.length > 0 ? generate({ applyNoList: this.applyNos, isGenerateDelivery: false }) : P0,
      //   this.callNos.length > 0 ? generateOrder({ callNoList: this.callNos, isGenerateDelivery: false }) : P0,
