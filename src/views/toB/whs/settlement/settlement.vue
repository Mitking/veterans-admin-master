    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          ...
          <div v-else-if="item.type==='PRICE_AUTOMATIC' || item.type==='PRICE' || item.type==='PRICE_FORM_AUTOMATIC'">{{ fen2yuan(row[item.prop]) }}</div>
...
import { getPage, deleteOrders, orderReviewer } from '@/api/tob/whssettlement'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [
        { title: '结算单号', prop: 'settleOrderNo', type: 'text', width: '300px' },
        { title: '结算日期', prop: 'settleDate', type: 'date' },
        { title: '组织', prop: 'settleOrgId', width: '300px', type: 'select', accountTypeShowFn: () => ([0, 3].includes(this.$store.getters.accountType)), defaultValue: [0, 3].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.orgId, selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
        { title: '结算门店', prop: 'settleStoreId', disabled: false, accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: [0, 3, 2].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.storeId, type: 'select', selectData: 'stores', width: '300px', fatherPropName: 'orgId', fatherProp: 'settleOrgId', selectOption: { label: 'name', value: 'id' }},
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['whs:settle:order:getSettleOrderInfo'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabledFn: this.financialAudited, disabled: 'isRow', type: 'primary', permission: ['whs:settle:order:saveSettleOrder'], width: 80 }
      ...
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['whs:settle:order:getPageList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['whs:settle:order:saveSettleOrder'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabledFn: this.financialAuditFn, disabled: 'ids', type: 'primary', permission: ['whs:settle:order:deleteSettleOrder'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviews, loading: false, disabled: 'ids', disabledFn: this.financialAuditFn, icon: 'el-icon-s-check', type: 'primary', permission: ['whs:settle:order:whsSettleOrderReview'], width: 80 }
      ],
      ids: [],
  ...
  methods: {
    ...
    reviews(btnItem, isSure = false) { // 审核
      ...
      orderReviewer({ 'ids': this.ids, auditStatu: '3' }).then(res => {
        this.message(res)
      }).finally(() => {
        btnItem.loading = false
      })
