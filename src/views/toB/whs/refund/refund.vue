    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          ...
          <div v-else-if="item.type==='select' || item.type==='selectState'">{{ item.propName?row[item.propName]:MenuGetPropName(item.selectData,row[item.prop],item.selectOption?item.selectOption:{ value: '', label: '' }) }}</div>
...
import { getPage, deleteOrders, orderReviewer } from '@/api/tob/whsrefund'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [
        { title: '退款单号', prop: 'whsRefundOrderNo', type: 'text', width: '300px' },
        { title: '退货日期', prop: 'whsRefundDate', type: 'date' },
        { title: '组织', prop: 'whsRefundOrgId', width: '300px', type: 'select', accountTypeShowFn: () => ([0, 3].includes(this.$store.getters.accountType)), defaultValue: [0, 3].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.orgId, selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
        { title: '退款门店', prop: 'whsRefundStoreId', disabled: false, accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: [0, 3, 2].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.storeId, type: 'select', selectData: 'stores', width: '300px', selectOption: { label: 'name', value: 'id' }, fatherPropName: 'orgId', fatherProp: 'whsRefundOrgId' },
        { title: '退款仓库', prop: 'whsRefundStorageId', disabled: false, type: 'select', defaultValue: 0, selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'whsRefundStoreId', fatherPropName: 'storeId', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['whs:refund:order:getWhsRefundOrderInfo'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabledFn: this.financialAudited, disabled: 'isRow', type: 'primary', permission: ['whs:refund:order:saveWhsRefundOrder'], width: 80 }
      ...
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['whs:refund:order:getPageList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['whs:refund:order:saveWhsRefundOrder'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabledFn: this.financialAuditFn, disabled: 'ids', type: 'primary', permission: ['whs:refund:order:batchDeleteWhsRefundOrder'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviews, loading: false, disabled: 'ids', disabledFn: this.financialAuditFn, icon: 'el-icon-s-check', type: 'primary', permission: ['whs:refund:order:whsRefundOrderReview'], width: 80 }
      ],
      ids: [],
