...
import { getPage, deleteOrders, orderReviewer, diffOrderInvalidation } from '@/api/tob/agencyreturn'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [
        { title: '退货单号', prop: 'returnNo', type: 'text', width: '300px' },
        ...
        { title: '退货门店', prop: 'storeId', disabled: false, accountTypeShowFn: () => ([0, 3, 2].includes(this.$store.getters.accountType)), defaultValue: [0, 3, 2].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.storeId, type: 'select', selectData: 'stores', width: '300px', fatherProp: 'orgId', selectOption: { label: 'name', value: 'id' }},
        { title: '退货仓库', prop: 'stockId', disabled: false, type: 'select', defaultValue: 0, selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, fatherProp: 'storeId', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['wms:consignmentReturn:info'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabledFn: this.financialAudited, disabled: 'isRow', type: 'primary', permission: ['wms:consignmentReturn:update'], width: 80 }
      ...
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['wms:consignmentReturn:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['wms:consignmentReturn:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabledFn: this.financialAuditFn, disabled: 'ids', type: 'primary', permission: ['wms:consignmentReturn:delete'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviews, loading: false, disabled: 'ids', disabledFn: this.financialAuditFn, icon: 'el-icon-s-check', type: 'primary', permission: ['wms:consignmentReturn:update:review'], width: 80 }
      ],
      ids: [],
