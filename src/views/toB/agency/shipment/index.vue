...
import { addOrSaveOrder, orderReviewer, orderNo, getDetails, saveOrder } from '@/api/tob/agencyshipment'
import WmsForm from '@/views/toB/components/WmsCompForm'
import Order from './shipment'
// import dayjs from 'dayjs'
export default {
  name: 'AgencyShipmentIndex',
  ...
  data() {
    const countCheck = (rule, value, callback) => { // 数量校验
      ...
      if (this.Mixin_tabDate[index].whetherCount && !this.Mixin_tabDate[index].consignmentNumber) {
    ...
    return {
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '出货单号', prop: 'shipmentNo', show: true, type: 'text', editShow: true, detailShow: true, readonly: true, addFatherProp: 'Req' },
        // { title: '出货日期', prop: 'shipmentDate', show: true, type: 'date', defaultValue: dayjs().format('YYYY-MM-DD'), width: 160, editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '组织', prop: 'orgId', show: true, tabHidden: true, defaultValue: [0, 3].includes(this.$store.getters.accountType) ? '' : this.$store.getters.orgId, width: '300px', type: 'select', readonly: false, editShow: true, tableHidden: [0, 3].includes(this.$store.getters.accountType) ? undefined : true, addFatherProp: 'Req', selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
        { title: '出货门店', prop: 'storeId', show: true, type: 'select', defaultValue: this.$store.getters.storeId, width: 200, selectData: 'stores', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId', readonly: this.$store.getters.accountType === 1, editShow: true, detailShow: true, addFatherProp: 'Req',
          rule: [{ required: true, message: '出货门店不能为空', trigger: 'change' }] },
        { title: '出货仓库', prop: 'stockId', show: true, type: 'select', width: 200, selectData: 'stockId', selectOption: { label: 'stockName', value: 'id' }, readonly: false, editShow: true, detailShow: true, addFatherProp: 'Req', fatherProp: 'storeId',
          rule: [{ required: true, message: '出货仓库不能为空', trigger: 'change' }] },
        { title: '代销商', width: 150, prop: 'customerId', propFromPageName: 'customerName', show: true, click: 'addItems', windowName: 'Customer', type: 'selectFromPage', formProData: { typeName: '代销商' }, editShow: true, detailShow: true, addFatherProp: 'Req',
          rule: [{ required: true, message: '代销商不能为空', trigger: 'change' }] },
        { title: '代销费率', prop: 'rate', show: true, tabHidden: true, type: 'RATE', float: { decimal: 1, max: 100 }, width: 160, editShow: true, defaultValue: '0', detailShow: true, addFatherProp: 'Req',
          rule: [{ validator: this.mixin_check_amount, message: '费率不能为空或0', trigger: 'change' }] },
        ...
        { title: '退货总金额', prop: 'returnTotalAmount', width: 120, show: true, type: 'PRICE', editShow: false, detailShow: false, addFatherProp: 'Req' },
        ...
        { title: '商品代码', prop: 'goodsCode', show: false, placeholder: '请选择', click: 'addItems', windowName: 'StoreProductIndex', formProData: { stopSale: 0, storeId: '', stockId: '' }, type: 'selectFromPage', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '商品名称', prop: 'goodsName', show: false, tabWidth: 200, placeholder: '请选择', click: 'addItems', windowName: 'StoreProductIndex', formProData: { stopSale: 0, storeId: '', stockId: '' }, type: 'selectFromPage', editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        ...
        { title: '代销价', prop: 'consignmentPrice', show: false, type: 'PRICE', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ validator: this.mixin_check_amount, message: '不能为0', trigger: 'change' }] },
        { title: '代销数量', prop: 'consignmentQuantity', show: false, type: 'weight_number', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ validator: this.mixin_check_amount, message: '不能为0', trigger: 'change' }] },
        { title: '金额', prop: 'totalAmount', show: false, type: 'PRICE_AUTOMATIC', design: this.mixin_auto_amount, arg: ['consignmentPrice', 'consignmentQuantity'], editShow: true, detailShow: true, addFatherProp: 'detailReqs' },
        { title: '个数', prop: 'consignmentNumber', show: false, placeholder: '计数商品必填', type: 'Number', fn_disabled: (row) => !row.whetherCount, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
  ...
  watch: {
    ...
    'Mixin_tabDate': {
      handler(n) {
        ...
        this.mixin_arr_edit_by_prop(this.table_data_arr, 'storeId', 'readonly', n.some(item => item.goodsCode !== '') || this.$store.getters.accountType === 1)
  ...
  methods: {
        ...
        case 'insert': // 新增
          ...
          if (data.detailReqs.length > 0) {
            ...
            data.detailReqs.map(item => {
              ...
              // item.shipmentId = data.Req.id
            })
          }
          data.shipmentReq = data.Req
          delete data.Req
          data.orgId = data.shipmentReq.orgId
    ...
    deleteDetail(item) {
      return new Promise(resolve => {
        this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('shipmentNo', this.table_data_arr)}「${this.currentRow.shipmentNo}」中的「${item.goodsName}」, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
