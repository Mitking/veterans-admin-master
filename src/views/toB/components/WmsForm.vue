  ...
  <div class="heightParent" :style="{height:(windowHeight-167)+'px'}">
    ...
    <div ref="wmsform" class="formFeather">
      ...
      <el-card v-if="$attrs.details" class="formCard">
        <wms-table ref="Table" :form_data="formData" :add_arr="$attrs.add_arr?$attrs.add_arr:[]" v-bind="$attrs" @onSubmit="onSubmit" @dataIsChange="dataIsChange=$event" v-on="$listeners" />
        <!--        <div>{{ $refs.Form  }}</div>-->
      </el-card>
    </div>
    <!--    底部    -->
...
<script>
import WmsFormItem from '@/views/toB/components/WmsFormItem'
import WmsDetailItem from '@/views/toB/components/WmsDetailItem'
import WmsTable from '@/views/toB/components/WmsTable'
// import { deleteOrders } from '@/api/wms/purdemand' current
export default {
  name: 'WmsForm',
  ...
  data() {
    return {
      formData: this.$attrs['current-row'] || {},
      collapseValue: '1',
      collapseNames: ['1'],
  ...
  methods: {
    onSubmit(arg, splitObj = null) {
      ...
      if (arg === 'purOrderSplit' && splitObj) { // 部分收货表格校验标记
        this.$listeners.purOrderSplit(splitObj)
      }
      const promise = []
      ...
      Promise.all(promise).then(res => {
        console.log('通过检验')
        if (arg === 'purOrderSplit' && splitObj) { // 部分收货表格校验标记
          this.$listeners.purOrderSplit(null)
        }
        ...
        if (this.$refs.Table) {
          ...
          this.$listeners.formSave(arg, temp, this.$refs.Table.deleteDetails, splitObj)
        } else {
