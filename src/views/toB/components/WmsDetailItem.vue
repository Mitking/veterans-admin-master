  ...
  <el-form ref="form" class="form-fa" :rules="rules" :model="form" label-width="100px">
    ...
    >
      ...
      <div v-else>
        ...
        <div v-if="item.type==='RATE'">{{ fen2yuan(form[item.prop])+'%' || '-' }}</div>
        <div v-if="item.type==='NUMBER_AUTOMATIC'">{{ comp_number_auto(item) || '-' }}</div>
        <div v-if="item.type==='PRICE_FORM_AUTOMATIC'">{{ fen2yuan(form[item.prop]) || '-' }}</div>
        <div v-if="item.type==='selectFromPage'">{{ form[item.propFromPageName] || '-' }}</div>
