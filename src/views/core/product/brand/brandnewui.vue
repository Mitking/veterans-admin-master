...
import { brandselect, branddelete, getBrandList } from '@/api/core/brand'
export default {
  name: 'Brand',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        ...
        { title: '品牌名', prop: 'name', type: 'text', width: '300px' }
      ],
      ...
      operateTableList: [
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['core:brand:update'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['core:brand:delete'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        ...
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['core:brand:add'], width: 80 }
      ],
  ...
  methods: {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) {
      ...
      brandselect(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将删除品牌「${this.currentRow.name}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        branddelete({ 'id': [this.currentRow.id] }).then(res => {
          this.message(res)
