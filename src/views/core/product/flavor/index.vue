...
import { addFlavor, updateFlavor, getFlavorList } from '@/api/core/flavor'
import CoreForm from '@/views/core/components/CoreForm'
import Order from './flavor'
export default {
  name: 'FlavorIndex',
  ...
  data() {
    return {
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        // { title: '所属组织', prop: 'orgId', show: true, defaultValue: [0, 3].includes(this.$store.getters.accountType) ? '' : this.$store.getters.orgId, width: '300px', type: 'select', editShow: [0, 3].includes(this.$store.getters.accountType), addFatherProp: 'Req', selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
        { title: '口感名', prop: 'name', show: true, type: 'text', editShow: true, addFatherProp: 'Req',
          rule: [{ required: true, message: '口感名必填', trigger: 'change' }] },
        { title: '描述', prop: 'remark', show: true, type: 'remark', width: 500, editShow: true, addFatherProp: 'Req' }
      ],
      details: [] // 详细数据
    }
  },
    ...
    formSave(arg, data = { Req: {}, detailReqs: [] }) { // 提交回调处理
      switch (arg.opt || arg) {
        ...
        case 'insert': // 新增
          ...
          break
        case 'edit': // 修改
          updateFlavor(data.Req).then(res => {
            if (res.code === 200 && arg.review) {
              this.review(res.data.id)
              return
            }
