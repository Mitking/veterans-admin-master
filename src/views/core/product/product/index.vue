...
export default {
  ...
  data() {
    ...
    //
    //   const index = propIndex === 3 ? 2 : 0
    //   // const basePrice = this.$refs.Comp.$refs.price_list.form.details[index].amount // 基准价
    //   const arr = this.$refs.Comp.$refs.price_list.form.details
    //   for (let i = 0; i < arr.length; i++) {
    //     if (propIndex === i) {
    //       if (arr[i].amount === '') { // 为空
    //         return callback(new Error('请填写价格'))
    //       }
    //       // if (propIndex > 1 && arr[i].algCode !== '') { // 算法验证
    //       //   const { data } = await calculatePrice({ price: basePrice, algCode: arr[i].algCode }).catch((err) => ({ data: err }))
    //       //
    //       //   if (arr[i].amount !== data.calculatePrice) {
    //       //     return callback(new Error('计算检验失败'))
    //       //   }
    //       // }
    //       callback()
    //     }
    ...
    // const checkAlgCode = async(rule, value, callback) => { // 算法检验
    //   const propIndex = Number(rule.field.substr(8, 1)) // 行号
    //
    //   // const basePrice = this.$refs.Comp.$refs.price_list.form.details[index].amount // 基准价
    //   const arr = this.$refs.Comp.$refs.price_list.form.details
    //   for (let i = 0; i < arr.length; i++) {
    //     if ((propIndex === 2 || propIndex === 3) && value === '') {
    //       return callback(new Error('该算法不能为空'))
    //     }
    //     callback()
    //   }
    // }
    ...
    return {
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '商品分类', prop: 'goodsCategoryId', tipMsgPath: '/core/proudct/goods-category', permission: 'core:productCategory:update', disabled: false, show: false, type: 'goodsCategory', selectData: 'categoryList', selectOption: { value: 'id', label: 'name' }, formShow: true, addFather: 'basics',
          rule: [{ required: true, message: '商品分类不能为空', trigger: 'change' }] },
        { title: '商品代码', prop: 'goodsCode', readonly: true, copy: true, show: true, type: 'text', formShow: true, addFather: 'basics', placeholder: '无需填写，系统生成',
          rule: [{ required: true, message: '请选择商品分类自动生成', trigger: 'change' }] },
        { title: '规格条码', prop: 'barcode', show: true, copy: true, placeholder: '系统生成,可修改', type: 'text', formShow: true, addFather: 'basics', readonly: false },
        ...
        { title: '品牌', tipMsgPath: '/core/proudct/brand', permission: 'core:brand:getList', prop: 'brandId', show: false, type: 'select', selectData: 'brandList', selectOption: { label: 'name', value: 'id' }, formShow: true, addFather: 'basics' },
        ...
        { title: '口感', prop: 'flavorId', tipMsgPath: '/core/proudct/flavor', permission: 'core:flavor:getList', show: false, type: 'select', selectData: 'FlavorList', selectOption: { value: 'id', label: 'name' }, formShow: true, addFather: 'basics' },
        { title: '保质期', prop: 'shelfLife', show: false, type: 'number', formItemWidth: 100, suffixType: 'text', suffix: '天', formShow: true, addFather: 'basics' },
        { title: '预警天数', prop: 'remindDay', show: false, type: 'number', float: { max: 100 }, formItemWidth: 100, suffixType: 'text', suffix: '天', formShow: true, addFather: 'basics' },

        { title: '税率', tipMsgPath: '/fin-tax-rate/set', permission: 'oms:finTaxRate:getList', prop: 'taxRateId', show: false, type: 'select', selectData: 'FinTaxRateList', selectOption: { value: 'id', label: 'name' }, formShow: true, addFather: 'basics' },
        ...
        { title: '重量', prop: 'weight', show: false, type: 'weight_number', formShow: true, addFather: 'storage', formItemWidth: 100 },
        { title: '皮重量', prop: 'tare', show: false, type: 'weight_number', formShow: true, addFather: 'storage', formItemWidth: 100 },
        { title: '净重量', prop: 'netWeight', show: false, type: 'weight_number', formShow: true, addFather: 'storage', formItemWidth: 100 },
        ...
        { title: '乐檬商品编码', prop: 'itemNum', show: false, placeholder: '不绑定乐檬不填写', decimal: 0, type: 'Number', float: { max: 9999999999999999 }, formShow: true, addFather: 'marketing' },
        ...
        { title: '非库存商品', prop: 'nonInventory', show: false, type: 'if_type', defaultValue: true, formShow: true, addFather: 'marketing' },
        ...
        { title: '是否分拣', prop: 'whetherSorted', show: false, type: 'if_type', defaultValue: true, formShow: true, addFather: 'marketing' },
        ...
        { title: '组合品id', prop: 'id', show: false, type: 'text', selectData: 'alg_team', formShow: false, addFather: 'combo_list' },
        ...
        { title: '销售计量单位', prop: 'saleMeasurementUnit', show: false, type: 'select', readonly: true, selectData: 'weight_type', formShow: true, addFather: 'combo_list',
  ...
  created() {},
  methods: {
    weightChange(v) { // 基本单位监听
      this.formDetails.whetherWeigh = (v === '4')
    },
    message(res) {
      if (res.code === 200) {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      // this.mixin_arr_edit_by_prop(this.table_data_arr, 'barcode', 'readonly', ['insert', 'copy'].includes(operateItem.prop))
          ...
          Promise.all(PArr)
            .then(res => {
              if (res.every(resCode => resCode.code === 200)) {
                ...
                temp.weightWeightShow = temp.weight ? this.g2kg(temp.weight) : ''
                temp.tareWeightShow = temp.tare ? this.g2kg(temp.tare) : ''
                temp.netWeightWeightShow = temp.netWeight ? this.g2kg(temp.netWeight) : ''
                this.priceList = [
                ...
                if (this.operateItem.prop !== 'copy') {
                  this.priceList.map((item, index) => {
                    ...
                    if (f.length >= 1) { // 过滤价格表
                      ...
                      if (item.priceType === '5') { // 零售价取算法组id
                        this.priceList[index].algCode = Number(this.priceList[index].algCode)
                      }
                    // if (this.operateItem.prop === 'copy') {
        ...
        case 'insert': // 新增
          {
            ...
            this.ArrFilter(this.table_data_arr, { addFather: 'unitConversionRateReq' }).map(item => {
              form_temp.unitConversionRateRes[item.prop] = typeof item.defaultValue === 'undefined' ? null : item.defaultValue
            })
            form_temp.weightWeightShow = ''
            form_temp.tareWeightShow = ''
            form_temp.netWeightWeightShow = ''
        ...
        case 'edit': case 'insert': case 'copy': // 修改,新增，复制
          {
            ...
            // temp.taxRateId = temp.addProductAchiveVo.taxRateId
            // temp.ifSyncToStore = temp.addProductAchiveVo.ifSyncToStore
            temp.addProductAchiveVo.weight = temp.addProductAchiveVo.weightWeightShow ? this.kg2g(temp.addProductAchiveVo.weightWeightShow) : ''
            temp.addProductAchiveVo.tare = temp.addProductAchiveVo.tareWeightShow ? this.kg2g(temp.addProductAchiveVo.tareWeightShow) : ''
            temp.addProductAchiveVo.netWeight = temp.addProductAchiveVo.netWeightWeightShow ? this.kg2g(temp.addProductAchiveVo.netWeightWeightShow) : ''
            temp.ifSyncToStore = true
            ...
            // delete temp.addProductAchiveVo.taxRateId
