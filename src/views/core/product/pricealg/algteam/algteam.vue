...
import { getPriceAlgTeamEntity, deletePriceAlgTeam, getAlgTeamPage } from '@/api/core/algteam'
export default {
  name: 'Order',
  components: { },
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '算法组名称', prop: 'teamName', type: 'text', width: '300px' }
      ],
  ...
  methods: {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) {
      ...
      getAlgTeamPage(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将删除算法分组「${this.currentRow.teamName}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deletePriceAlgTeam({ 'ids': [this.currentRow.id] }).then(res => {
          this.message(res)
