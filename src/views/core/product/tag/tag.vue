<template>
  <div>
    ...
    <el-table
      ...
      style="margin:0 24px;"
      :style="{width:minxin_width_total(ArrFilter($attrs['form-data'],{show:true}))}"
...
import { getTagPage, deleteTag } from '@/api/core/tag'
export default {
  name: 'Tag',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        ...
        { title: '标签', prop: 'tagName', type: 'text', width: '300px' }
      ],
      ...
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['core:brand:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['core:brand:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['core:brand:getList'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['core:brand:getList'], width: 80 }
      ],
  ...
  methods: {
    ...
    search() {
      ...
      getTagPage(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除品牌「${this.currentRow.name}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteTag({ 'id': this.currentRow.id }).then(res => {
          this.message(res)
