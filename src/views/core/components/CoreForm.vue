<template>
  <div class="heightParent" :style="{height:$attrs.is_child?(windowHeight-180)+'px':(windowHeight-167)+'px'}">
    ...
    <div class="formFeather">
      <!--   基础信息   -->
      <el-card class="formCard my-class-core my-class-line">
        <el-collapse :value="collapseNames" @change="collapseValue = $event[0]">
          <el-collapse-item name="1">
            <div slot="title" style="font-size:1.05rem">基础信息</div>
            <core-form-item v-if="$attrs['operate-item'].prop!=='detail'" ref="Form" style="padding-bottom: 5px" class="my-class-edit" v-bind="$attrs" @formChange="formIsChange=($event===false?false:true)" v-on="$listeners" />
            <core-detail-item v-if="$attrs['operate-item'].prop==='detail'" ref="Form" class="my-class-detail" v-bind="$attrs" v-on="$listeners" />
          </el-collapse-item>
        </el-collapse>
      </el-card>
      <!--  详情备注  -->
      ...
      <el-card v-if="$attrs.details" class="formCard">
        <core-table ref="Table" v-bind="$attrs" v-on="$listeners" />
      </el-card>
    </div>
    <!--    底部    -->
...
<script>
import CoreFormItem from '@/views/core/components/CoreFormItem'
import CoreDetailItem from '@/views/core/components/CoreDetailItem'
import CoreTable from '@/views/core/components/CoreTable'
// import { deleteOrders } from '@/api/core/purdemand'
export default {
  name: 'CoreForm',
  components: { CoreFormItem, CoreTable, CoreDetailItem },
  props: {
    loading: {
      type: Boolean,
      defaultValue: () => false
...
.my-class-core >>> .el-collapse-item__header{
  border:none;
  border-bottom-color: #e6ebf5;
 }
.my-class-core >>> .el-collapse-item__header.collapse-transition{
  border:none;
}
.my-class-core >>> .el-collapse-item__content{
  padding-bottom: 0
}
.my-class-core >>> .el-collapse{
  border:none
}
.my-class-core >>> .el-collapse-item__wrap{
  border:none
}
