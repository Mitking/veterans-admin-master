...
import { getStockPage, deleteStockById, getStockList } from '@/api/core/stock'
export default {
  name: 'Order',
  components: { },
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '仓库名', prop: 'stockName', type: 'text', width: '300px' },
        ...
        { title: '仓库分组', prop: 'stockGroupId', type: 'select', selectData: 'stock_group', defaultValue: 0, selectOption: { label: 'groupName', value: 'id' }, width: '300px' }
      ],
      ...
      operateTableList: [
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['core:stock:update'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['core:stock:delete'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['core:stock:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['core:stock:add'], width: 80 }
  ...
  methods: {
    message(res) {
      if (res.code === 200) {
        this.search()
        this.$success('操作成功')
        getStockList({}).then(res => { // 字典表强制更新
          if (res.code === 200) {
            this.$store.dispatch('enum/add_enum', { 'stockId': res.data })
          }
        })
      } else {
        this.$error(res.data)
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) {
      ...
      getStockPage(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除仓库「${this.currentRow.stockName}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteStockById({ 'ids': [this.currentRow.id] }).then(res => {
          this.message(res)
