...
export default {
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        { title: '所属组织', prop: 'orgId', defaultValue: [0, 3].includes(this.$store.getters.accountType) ? 0 : this.$store.getters.orgId, width: '300px', type: 'select', accountTypeShowFn: () => ([0, 3].includes(this.$store.getters.accountType)), selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['core:store:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['core:store:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['core:store:getList'], width: 80 }
        ...
        // { title: '复制', show: true, prop: 'copy', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['ums:role:grantMenu', 'ums:role:page'], width: 80 },
        // { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['ums:role:page'], width: 80 }
      ],
  ...
  created() {
    ...
    //
    // 获取门店类型列表
    // if (this.$store.state.enum.enumList.store_group.length === 0) {
    //   getGroupList().then(res => {
    //     if (res.code === 200) {
    //       this.$store.dispatch('enum/add_enum', { 'store_group': res.data }).then(res => { // 异步处理
    //         if (res) {
    //           this.search()
    //         }
    //       })
