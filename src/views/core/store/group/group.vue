...
import { getStoreGroupPage, deleteStoreGroupById, getGroupList } from '@/api/core/store'
export default {
  name: 'Order',
  components: { },
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        ...
        // { title: '组织', prop: 'orgId', type: 'select', selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, defaultValue: 0, width: '200px' },
      ...
      operateTableList: [
        { title: '修改', prop: 'edit', show: true, click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['core:storeGroup:update'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['core:storeGroup:delete'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['core:storeGroup:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['core:storeGroup:add'], width: 80 }
        // { title: '复制', prop: 'copy', click: this.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['core:storeGroup:getList'], width: 80 },
      ],
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        { title: '主键', prop: 'id', show: false, disabled: true, type: 'text', defaultValue: null, editShow: true },
        { title: '分组名称', prop: 'groupName', show: true, type: 'text', editShow: true },
        // { title: '分组代码', prop: 'groupCode', show: true, type: 'text', editShow: true },
        ...
        { title: '备注', prop: 'remark', show: true, type: 'text', width: '600', editShow: true }
      ]
    }
  },
  computed: {},
  ...
  methods: {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) {
      ...
      getStoreGroupPage(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除分组「${this.currentRow.groupName}」, 是否继续?`, '提示', {
        ...
        deleteStoreGroupById({ 'ids': [this.currentRow.id] }).then(res => {
          this.message(res)
