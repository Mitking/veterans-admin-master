...
import { getSupplierGroupList, deleteSupplierGroup, getSupplierGroupPage } from '@/api/core/supplier'
export default {
  name: 'Order',
  components: { },
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        ...
        // { title: '组织', prop: 'orgId', type: 'select', selectData: 'orgs', defaultValue: 0, width: '200px', fatherProp: 'accountId' },
        { title: '市场名称', prop: 'groupName', type: 'text', width: '300px' }
        // { title: '启用禁用', prop: 'enable', defaultValue: true, type: 'checkbox', width: '300px' }
      ],
      ...
      operateTableList: [
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['core:supplierGroup:update'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.del, loading: false, icon: 'el-icon-delete', disabled: 'isRow', type: 'primary', permission: ['core:supplierGroup:delete'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['core:supplierGroup:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['core:supplierGroup:add'], width: 80 }
  ...
  methods: {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) {
      ...
      getSupplierGroupPage(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将删除市场「${this.currentRow.groupName}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // this.operateItem.loading = true
        deleteSupplierGroup({ 'ids': [this.currentRow.id] }).then(res => {
          this.message(res)
