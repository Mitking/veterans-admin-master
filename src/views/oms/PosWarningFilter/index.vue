...
import { posWarningFilterAdd, posWarningFilterUpdate } from '@/api/oms/PosWarningFilter'
import CoreForm from '@/views/core/components/CoreForm'
import Order from './PosWarningFilter.vue'
export default {
  name: 'PosWarningFilter',
  ...
  data() {
    return {
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '卡号', prop: 'cardNo', width: 200, show: true, type: 'text', addFatherProp: 'Req', editShow: true, detailShow: true,
          rule: [{ required: true, message: '卡号必填', trigger: 'change' }] },
        { title: '门店名', prop: 'storeName', show: true, width: 200, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '创建时间', prop: 'createTime', width: 150, show: true, type: 'text', addFatherProp: 'Req', editShow: false, detailShow: true }
      ],
      details: [] // 详细数据
    }
  },
  ...
  methods: {
    ...
    formSave(arg, data = { Req: {}, detailReqs: [] }) { // 提交回调处理
      switch (arg) {
        ...
        case 'insert': // 新增,复制
          data.Req.id = null
          posWarningFilterAdd(data.Req).then(res => {
            this.message(res)
          }).finally(() => {
            this.loading = false
          })
          break
        case 'edit': // 修改
          posWarningFilterUpdate(data.Req).then(res => {
