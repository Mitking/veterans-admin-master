...
import { getListForInvoice } from '@/api/oms/fin/suppliersettle'
export default {
  name: 'SupplierInvioceSelectIndex',
  components: { },
  // 全局常用字段
  mixins: [MixinGlobal],
  data() {
    return {
      searchList: [],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finSupplierSettlement:getList'], width: 80 }
      ],
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        { title: '主键', prop: 'id', show: false, disabled: true, type: 'text', defaultValue: null, editShow: true, detailShow: false, addFatherProp: 'Req' },
        { title: '结算单号', prop: 'supplierSettlementNo', show: true, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req', readonly: true },
        { title: '结算金额', prop: 'settlementAmount', width: 100, show: true, type: 'PRICE', editShow: true, detailShow: true, addFatherProp: 'Req' },
  ...
  watch: {
    ...
    '$attrs.select_window_name': {
      handler(n) {
        if (n === 'SupplierInvioceSelectIndex' && this.$attrs.add_arr.length === 0) {
          console.log('选择窗口打开了')
  ...
  methods: {
    ...
    search() { // 操作
      ...
      getListForInvoice(searchForm).then(res => {
