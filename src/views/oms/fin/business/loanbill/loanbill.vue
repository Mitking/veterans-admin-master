...
import { getPage, deleteOrder, review } from '@/api/oms/fin/loanbill'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '借款单号', prop: 'loanBillNo', type: 'text', width: '300px' },
        { title: '借款日期', prop: 'loanDate', type: 'date', width: '300px' },
        { title: '借款人', prop: 'loanPerson', type: 'text', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finLoanBillDetails:getDetails'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finLoanBill:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        ...
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finLoanBill:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finLoanBill:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finLoanBill:update:review'], width: 80 }
  ...
  methods: {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'loanBillNos': this.selectRows.map(item => item.loanBillNo) }).then(res => {
          this.message(res)
