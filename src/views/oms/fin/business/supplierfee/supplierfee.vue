...
import { getPage, deleteOrder, review } from '@/api/oms/fin/supplierfee'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '供应商费用单号', prop: 'supplierFeeNo', type: 'text', width: '300px' },
        { title: '费用名称', prop: 'feeNameId', defaultValue: 0, type: 'feeName', selectData: 'feeSelectListLast', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finSupplierFeeDetails:getDetails'], width: 80 },
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finSupplierFee:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: !this.$attrs.is_child, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finSupplierFee:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.is_child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finSupplierFee:add'], width: 80 },
        { title: '删除', show: !this.$attrs.is_child, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finSupplierFee:delete'], width: 80 },
        ...
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finSupplierFee:update:review'], width: 80 }
  ...
  watch: {
    ...
    '$attrs.select_window_name': {
      handler(n) {
        if (n === 'SupplierFeeIndex' && this.$attrs.add_arr.length === 0) {
          console.log('SupplierFeeIndex选择窗口打开了')
  ...
  methods: {
    ...
    windowSelectable(row) {
      if (this.$attrs.is_child) {
        return !this.$attrs.table_goods_code_arr.includes(row.supplierFeeNo)
      }
      return true
    },
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'supplierFeeNos': this.selectRows.map(item => item.supplierFeeNo) }).then(res => {
          this.message(res)
