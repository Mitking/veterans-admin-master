    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          ...
          >
            <div>
              {{ row[item.prop].map(itemZ=>MenuGetPropName('stores',itemZ,{ value: 'id', label: 'name' })).toString() | sizeFilter(10) }}
            </div>
          </el-tooltip>
          <!--          v-else-->
...
import { getPage, deleteOrders, review, copyAutoPrice } from '@/api/wms/purprice'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      ...
      searchList: [ // 搜索栏项目
        ...
        { title: '单据号', prop: 'autoPriceNo', type: 'text', width: '300px' },
        ...
        { title: '定价日期', prop: 'priceDate', type: 'date', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finAutoPriceDetails:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finAutoPrice:update'], width: 80 },
        { title: '复制', show: true, prop: 'copy', click: this.copy, loading: false, icon: 'el-icon-copy-document', disabled: 'isRow', type: 'primary', permission: ['oms:finAutoPrice:copy'], width: 80 },
        { title: '导出', show: true, prop: 'export', click: this.exportFn, loading: false, icon: 'el-icon-download', disabled: 'isRowNoWillRev', type: 'primary', permission: ['oms:finAutoPriceDetails:print'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finAutoPrice:getList'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finAutoPrice:review'], width: 80 }
  ...
  methods: {
    ...
    copy(e, isSure = false) {
      ...
      copyAutoPrice({ autoPriceNo: this.currentRow.autoPriceNo }).then(res => {
        this.message(res)
      }).finally(() => {
        e.loading = false
      })
    ...
    del(operateItem = {}) {
      ...
      this.$confirm(`此操作将永久删除${this.mix_comp_find_order_title_name('purPriceOrderNo', this.$attrs['form-data'])}「${this.currentRow.purPriceOrderNo}」, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
