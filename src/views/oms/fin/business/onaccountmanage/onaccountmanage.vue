...
import { getPage, deleteOrder } from '@/api/oms/fin/onaccountmanage'
...
export default {
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        // { title: '门店', show: false, width: 200, prop: 'storeId', type: 'select', selectData: 'stores', selectOption: { label: 'name', value: 'id' }},
        ...
        { title: '挂账人', prop: 'name', type: 'text', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:onaccount:page'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:onaccount:update'], width: 80 },
        { title: '还款', show: true, prop: 'repay', click: this.$listeners.openModel, loading: false, icon: 'el-icon-money', disabled: 'isRow', type: 'primary', permission: ['oms:onaccount:repay'], width: 80 },
        { title: '还款流水', show: true, prop: 'repaylog', click: this.$listeners.openModel, loading: false, icon: 'el-icon-folder-remove', disabled: 'isRow', type: 'primary', permission: ['oms:onaccount:repay:log'], width: 80 },
        { title: '挂账流水', show: true, prop: 'onaccountlog', click: this.$listeners.openModel, loading: false, icon: 'el-icon-folder-add', disabled: 'isRow', type: 'primary', permission: ['oms:onaccount:log'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finLoanBill:getList'], width: 80 },
        { title: '新增挂账人', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:onaccount:append'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:onaccount:delete'], width: 80 }
      ],
      ids: [],
      selectRows: [],
  ...
  methods: {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ ids: this.selectRows.map(item => item.id) }).then(res => {
          this.message(res)
