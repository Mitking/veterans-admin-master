...
import { getAssociateOrder } from '@/api/oms/fin/feemanage'
...
export default {
  ...
  data() {
    return {
      ...
      operateTableList: [
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRow', type: 'primary', permission: ['oms:finFeeNameAssociate:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finFeeNameAssociate:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finFeeNameAssociate:add'], width: 80 }
  ...
  created() {
    ...
    // getFeeList().then(res => { // 获取费用字典
    //   if (res.code === 200) {
    //     this.$store.dispatch('enum/add_enum', { 'FeeList': res.data }).then(() => {
    //       this.search()
    //     })
    //   }
    // })
  ...
  methods: {
    ...
    search(operateItem = { prop: '' }, pageSize = null, pageNum = null) {
      ...
      getAssociateOrder(searchForm).then(res => {
        this.currentRow = { id: '' }
        if (res.code === 200) {
