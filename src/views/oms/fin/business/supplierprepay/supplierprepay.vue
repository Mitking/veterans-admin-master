...
import { getPage, deleteOrder, review } from '@/api/oms/fin/supplierprepay'
export default {
  name: 'Order',
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '预付款单号', prop: 'supplierPrepayNo', type: 'text', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finSupplierPrepay:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finSupplierPrepay:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finSupplierPrepay:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finSupplierPrepay:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finSupplierPrepay:delete'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', disabledFn: this.reviewDisabledFn, type: 'primary', permission: ['oms:finSupplierPrepay:update:review'], width: 80 }
      ],
      ids: [],
  ...
  methods: {
    reviewDisabledFn() {
      return this.mixin_rows.some(item => !item.preAmount)
    },
