...
import { addOrder, updateOrder, getSendNumber, review } from '@/api/oms/fin/supplierprepay'
import OmsCompForm from '@/views/oms/components/OmsCompForm'
import Order from './supplierprepay'
import dayjs from 'dayjs'
export default {
  name: 'SupplierPrepayIndex',
  ...
  data() {
    return {
      form_bottons: [
        { title: '审核', mix_comp_buttons_show: this.mix_comp_buttons_show, editShowArgs: { auditStatus: '1' }, detailShowArgs: { auditStatus: '1' }, prop: 'review', loading: false, icon: 'el-icon-s-check', type: 'primary', permission: ['oms:finSupplierPrepay:update:review'] }
      ],
      loading: false,
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '预付款单号', prop: 'supplierPrepayNo', width: 250, show: true, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req', readonly: true },
        { title: '付款日期', prop: 'busDate', show: true, type: 'date', width: 150, editShow: true, detailShow: true, addFatherProp: 'Req', defaultValue: dayjs().format('YYYY-MM-DD'),
        ...
        { title: '预付款金额', prop: 'preAmount', width: 120, show: true, type: 'PRICE', editShow: true, detailShow: true, addFatherProp: 'Req',
          rule: [{ required: true, message: '金额不能为空', trigger: 'change' }] },
        { title: '付款方式', prop: 'payMode', show: true, type: 'select', width: 150, selectData: 'charge_mode', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '付款银行', prop: 'bankName', show: true, width: 200, type: 'select', selectData: 'bank', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '付款账户', prop: 'bankNo', show: true, type: 'text', width: 200, editShow: true, detailShow: true, addFatherProp: 'Req' },
        ...
        // { title: '现金银行', prop: 'bank', show: true, type: 'select', selectData: 'bank', editShow: true, detailShow: true, addFatherProp: 'Req' },
        ...
        // { title: '分拣入库单号', prop: 'supplierPrepayNo', show: false, type: 'text', editShow: true, detailShow: true, addFatherProp: 'detailReqs', tableHidden: true },
  ...
  methods: {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      if (['detail', 'edit'].includes(operateItem.prop)) { // 详情，修改
        ...
        getSendNumber({ type: 'CW_GY_YF' }).then(res => {
          if (res.code === 200) {
    ...
    //     this.$confirm(`此操作将永久删除单号「${item.supplierPrepayNo}」中的「${item.goodsName}」, 是否继续?`, '提示', {
    //       confirmButtonText: '确定',
    //       cancelButtonText: '取消',
    //       type: 'warning'
    //     }).then(() => {
