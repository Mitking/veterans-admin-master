...
import { getPage, deleteOrder, review } from '@/api/oms/fin/charge'
export default {
  name: 'Order',
  components: {},
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '收款单号', prop: 'finChargeNo', type: 'text', width: '300px' },
        // { title: '审核人id', prop: 'auditPersonId', type: 'text', width: '300px' },
        // { title: '审核状态', prop: 'auditStatus', type: 'select', selectData: 'audit_status', width: '300px' },
        { title: '加盟商', prop: 'franchiseeId', defaultValue: 0, width: '300px', type: 'select', selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }},
        { title: '收款日期', prop: 'chargeDate', type: 'date', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finChargeSlipDetails:getDetails', 'oms:finFranchiseePrepaid:query'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finChargeSlip:update', 'oms:finFranchiseePrepaid:query'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finChargeSlip:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finChargeSlip:add', 'oms:finFranchiseePrepaid:query'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finChargeSlip:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finChargeSlip:update:review'], width: 80 }
  ...
  methods: {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'finChargeNos': this.mixin_rows.map(item => item.finChargeNo) }).then(res => {
          this.message(res)
