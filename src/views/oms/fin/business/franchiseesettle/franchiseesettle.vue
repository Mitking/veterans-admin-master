...
import { getPage, deleteOrder, review } from '@/api/oms/fin/franchiseesettle'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '加盟商结算单号', prop: 'franchiseeSettleNo', type: 'text', width: '300px' },
        { title: '加盟商', prop: 'settleOrgId', type: 'select', defaultValue: 0, selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, width: '300px' },
        { title: '审核状态', prop: 'auditStatus', disabled: false, defaultValue: 0, type: 'select', selectData: 'audit_status', width: '300px' },
        { title: '结算日期', prop: 'settleDate', type: 'date', width: '300px' },
        { title: '制单人', prop: 'creatorName', reallyProp: 'creator', type: 'selectFromPage', selectForm: 'User', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finFranchiseeSettleDetails:getDetails', 'oms:finFranchiseeSettle:query'], width: 80 },
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finFranchiseeSettle:update', 'oms:finFranchiseeSettle:query'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: !this.$attrs.is_child, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finFranchiseeSettle:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.is_child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finFranchiseeSettle:add', 'oms:finFranchiseeSettle:query'], width: 80 },
        { title: '删除', show: !this.$attrs.is_child, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finFranchiseeSettle:delete'], width: 80 },
        ...
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finFranchiseeSettle:update:review'], width: 80 }
  ...
  watch: {
    ...
    '$attrs.select_window_name': {
      handler(n) {
        if (n === 'FinFranchiseeSettleIndex' && this.$attrs.add_arr.length === 0) {
          console.log('选择窗口打开了')
  ...
  methods: {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'franchiseeSettleNos': this.mixin_rows.map(item => item.franchiseeSettleNo) }).then(res => {
          this.message(res)
