...
import { getDetails, addOrder, updateOrder, deleteDetails, getSendNumber, review } from '@/api/oms/fin/expense'
import OmsCompForm from '@/views/oms/components/OmsCompForm'
import Order from './expense'
...
export default {
  ...
  data() {
    const that = this
    const payAmountCheck = function(rule, value, callback) {
      if (value > that.payAmountMaxFunc()) {
        callback(new Error('已付金额不能大于总金额'))
      } else {
        callback()
      }
    }
    return {
      ...
      form_bottons: [
        { title: '审核', mix_comp_buttons_show: this.mix_comp_buttons_show, editShowArgs: { auditStatus: '1' }, detailShowArgs: { auditStatus: '1' }, prop: 'review', loading: false, icon: 'el-icon-s-check', type: 'primary', permission: ['oms:finExpense:update:review'] }
      ],
      loading: false,
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '报销单号', prop: 'expenseNo', show: true, width: 250, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req', readonly: true },
        { title: '报销日期', prop: 'expenseDate', width: 150, show: true, type: 'date', editShow: true, detailShow: true, addFatherProp: 'Req', defaultValue: dayjs().format('YYYY-MM-DD'),
          rule: [{ required: true, message: '报销日期不能为空', trigger: 'change' }] },
        { title: '报销人', prop: 'expensePerson', show: true, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '报销金额', prop: 'sumAmount', width: 100, show: true, type: 'PRICE_AUTOMATIC', design: this.mixin_auto_total_amount, arg: ['expenseAmount'], editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '已付金额', prop: 'payAmount', width: 100, show: false, type: 'PRICE', maxValue: this.payAmountMaxFunc, editShow: false, detailShow: false, addFatherProp: 'Req',
          rule: [{ validator: payAmountCheck, trigger: 'change' }] },
        { title: '未付金额', prop: 'noPayAmount', width: 100, show: false, type: 'PRICE_AUTOMATIC', design: this.mixin_auto_form_diff, arg: ['sumAmount', 'payAmount'], editShow: false, detailShow: false, addFatherProp: 'Req' },
        { title: '付款方式', prop: 'payMode', width: 150, formItemWidth: 150, show: false, type: 'select', selectData: 'charge_mode', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '审核状态', prop: 'auditStatus', width: 100, show: true, type: 'selectState', selectData: 'audit_status', editShow: false, detailShow: true, addFatherProp: 'Req' },
        ...
        { title: '事由', prop: 'reason', show: false, type: 'text', editShow: true, detailShow: false, addFatherProp: 'Req' },
        ...
        { title: '报销单号', prop: 'expenseNo', show: false, type: 'text', editShow: true, detailShow: true, addFatherProp: 'detailReqs', tableHidden: true },
        ...
        // { title: '参与人', width: 150, prop: 'participant', show: false, type: 'text', editShow: true, detailShow: true, tableHidden: true, addFatherProp: 'Req' },
        { title: '报销金额', prop: 'expenseAmount', show: false, type: 'PRICE', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ validator: this.mixin_check_amount, message: '不能为空或0', trigger: 'change' }] },
        { title: '发生日期', prop: 'incidentTime', tableWidth: 180, show: false, type: 'date', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
  ...
  methods: {
    payAmountMaxFunc() {
      return this.mixin_form.sumAmount
    },
    message(res) {
      if (res.code === 200) {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      if (['detail', 'edit'].includes(operateItem.prop)) { // 详情，修改
        getDetails({ 'expenseNo': this.currentRow.expenseNo }).then(res => {
          if (res.code === 200) {
        ...
        this.details = []
        getSendNumber({ type: 'CW_FY_BX' }).then(res => {
          if (res.code === 200) {
        ...
        case 'insert': // 新增
          ...
          if (data.detailReqs.length > 0) {
            data.detailReqs.map(item => {
              item.id = null
              item.expenseNo = data.Req.expenseNo
            })
          }
          data.expenseReq = data.Req
