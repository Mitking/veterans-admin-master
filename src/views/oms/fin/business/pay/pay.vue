...
import { getPage, deleteOrder, review } from '@/api/oms/fin/pay'
export default {
  name: 'Order',
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        ...
        { title: '付款单号', prop: 'orderNo', type: 'text', width: '300px' },
        { title: '供应商', prop: 'supplierName', placeholder: '全部', reallyProp: 'supplierId', type: 'selectFromPage', selectForm: 'SupplierIndex', width: '300px' },
        { title: '付款日期', prop: 'payDate', type: 'date', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finPayOrderDetails:getDetails', 'oms:finSupplierPrepay:query'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finPayOrder:update', 'oms:finSupplierPrepay:query'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finPayOrder:getList'], width: 80 },
        { title: '新增', show: true, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finPayOrder:add', 'oms:finSupplierPrepay:query'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finPayOrder:delete'], width: 80 },
        ...
        { title: '审核', show: true, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finPayOrder:update:review'], width: 80 }
  ...
  methods: {
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'orderNos': this.mixin_rows.map(item => item.orderNo) }).then(res => {
          this.message(res)
