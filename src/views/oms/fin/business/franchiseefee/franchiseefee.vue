...
import { getPage, deleteOrder, review } from '@/api/oms/fin/franchiseefee'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '加盟商费用单号', prop: 'franchiseeFeeNo', type: 'text', width: '300px' },
        { title: '费用名称', prop: 'feeNameId', defaultValue: 0, type: 'feeName', selectData: 'feeSelectListLast', width: '300px', selectOption: { label: 'feeName', value: 'id' }},
        { title: '加盟商', prop: 'franchiseeOrgId', type: 'select', defaultValue: 0, selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, width: '300px' },
        { title: '加盟门店', prop: 'storeId', defaultValue: 0, type: 'select', selectData: 'stores', selectOption: { label: 'name', value: 'id' }, fatherProp: 'franchiseeOrgId', fatherPropName: 'orgId', width: '300px' },
        { title: '业务日期', prop: 'busDate', type: 'date', width: '300px' },
        { title: '审核状态', prop: 'auditStatus', type: 'select', defaultValue: 0, selectData: 'audit_status', width: '300px' },
        ...
        { title: '结算状态', disabled: false, prop: 'settleState', type: 'select', selectData: 'settle_state', defaultValue: 0, width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:finFranchiseeFeeDetails:getDetails'], width: 80 },
        { title: '修改', show: !this.$attrs.is_child, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:finFranchiseeFee:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: !this.$attrs.is_child, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:finFranchiseeFee:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.is_child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:finFranchiseeFee:add'], width: 80 },
        { title: '删除', show: !this.$attrs.is_child, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finFranchiseeFee:delete'], width: 80 },
        ...
        { title: '审核', show: !this.$attrs.is_child, prop: 'edit', click: this.reviews, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:finFranchiseeFee:update:review'], width: 80 }
  ...
  watch: {
    ...
    '$attrs.select_window_name': {
      handler(n) {
        if (n === 'FinFranchiseeFeeIndex' && this.$attrs.add_arr.length === 0) {
          console.log('选择窗口打开了')
  ...
  methods: {
    ...
    windowSelectable(row) {
      if (this.$attrs.is_child) {
        return !this.$attrs.table_goods_code_arr.includes(row.franchiseeFeeNo)
      }
      return true
    },
    ...
    dels(operateItem = {}) {
      ...
      this.$confirm(`确认删除所选项目吗?`, '提示', {
        ...
        deleteOrder({ 'franchiseeFeeNos': this.mixin_rows.map(item => item.franchiseeFeeNo) }).then(res => {
          this.message(res)
