...
import { getDetails, addOrder, updateOrder, deleteDetails, getSendNumber, review } from '@/api/oms/fin/franchiseefee'
import OmsCompForm from '@/views/oms/components/OmsCompForm'
import OmsForm from '@/views/oms/components/OmsForm'
import Order from './franchiseefee'
import dayjs from 'dayjs'
export default {
  name: 'FinFranchiseeFeeIndex',
  ...
  data() {
    return {
      ...
      form_bottons: [
        { title: '审核', mix_comp_buttons_show: this.mix_comp_buttons_show, editShowArgs: { auditStatus: '1' }, detailShowArgs: { auditStatus: '1' }, prop: 'review', loading: false, icon: 'el-icon-s-check', type: 'primary', permission: ['oms:finFranchiseeFee:update:review'] }
      ],
      loading: false,
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '费用单号', prop: 'franchiseeFeeNo', width: 250, show: true, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req', readonly: true },
        { title: '费用日期', prop: 'busDate', show: true, width: 150, type: 'date', editShow: true, detailShow: true, addFatherProp: 'Req', defaultValue: dayjs().format('YYYY-MM-DD'),
          rule: [{ required: true, message: '业务日期不能为空', trigger: 'change' }] },
        { title: '组织', prop: 'franchiseeOrgId', width: 200, show: true, type: 'select', selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, selectOutFilter: [this.$store.getters.orgId], editShow: true, detailShow: false, addFatherProp: 'Req',
          rule: [{ required: true, message: '加盟组织不能为空', trigger: 'change' }] },
        { title: '门店', prop: 'storeId', width: 200, show: true, type: 'select', selectData: 'stores', selectOption: { label: 'name', value: 'id' }, fatherPropName: 'orgId', fatherProp: 'franchiseeOrgId', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '费用金额', prop: 'sumFeeAmount', width: 100, show: true, type: 'PRICE_AUTOMATIC', design: this.mixin_auto_total_amount, arg: ['feeAmount'], editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '审核状态', prop: 'auditStatus', width: 100, show: true, type: 'selectState', readonly: true, defaultValue: '1', selectData: 'audit_status', editShow: true, detailShow: true, addFatherProp: 'Req' },
        ...
        { title: '加盟商费用单号', prop: 'franchiseeFeeNo', show: false, type: 'text', editShow: true, detailShow: true, addFatherProp: 'detailReqs', tableHidden: true },
        { title: '业务类别', prop: 'feeCategory', show: false, type: 'select', selectData: 'business_type', editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '业务类型(隐藏)', prop: 'businessType', show: false, type: 'select', selectData: 'business_cate', editShow: true, detailShow: true, addFatherProp: 'detailReqs', tableHidden: true,
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        ...
        { title: '费用金额', prop: 'feeAmount', show: false, type: 'PRICE', float: { minus: true, max: 999999 }, inputFunc: this.feeAmountDealFn, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '发生日期', prop: 'whenTime', show: false, type: 'date', tableWidth: 160, editShow: true, detailShow: true, addFatherProp: 'detailReqs',
          rule: [{ required: true, message: '不能为空', trigger: 'change' }] },
        { title: '备注', prop: 'remark', show: false, tableWidth: 300, type: 'text', editShow: true, detailShow: true, addFatherProp: 'detailReqs' }
      ],
      details: [] // 详细数据
    }
  },
  ...
  methods: {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      if (['detail', 'edit'].includes(operateItem.prop)) { // 详情，修改
        getDetails({ 'franchiseeFeeNo': this.currentRow.franchiseeFeeNo }).then(res => {
          if (res.code === 200) {
        ...
        this.details = []
        getSendNumber({ type: 'CW_JM_FY' }).then(res => {
          if (res.code === 200) {
        ...
        case 'insert': // 新增
          ...
          if (data.detailReqs.length > 0) {
            data.detailReqs.map(item => {
              item.id = null
              item.franchiseeFeeNo = data.Req.franchiseeFeeNo
            })
          }
          data.franchiseeFeeReq = data.Req
          delete data.Req
          if (arg.review) { // 新增并审核
    ...
    deleteDetail(item) {
      return new Promise(resolve => {
        this.$confirm(`此操作将永久删除单号「${item.franchiseeFeeNo}」中的项目, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
