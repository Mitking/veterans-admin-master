...
<script>
import OmsForm from '@/views/oms/components/OmsinvoiceForm'
import OmsAddCardForm from '@/views/oms/components/OmsInvoiceAddCardForm'
export default {
  name: 'OmsInvoiceCompForm',
  ...
  methods: {
    ...
    openSelectWindow(e) {
      ...
      if (['FinFranchiseeSettleIndex', 'InvioceSelectIndex'].includes(this.window_name)) {
        this.table_goods_code_arr = e.addedList.map(item => item.franchiseeSettleNo)
      }

      if (['SupplierSettleIndex', 'SupplierInvioceSelectIndex'].includes(this.window_name)) {
        this.table_goods_code_arr = e.addedList.map(item => item.supplierSettlementNo)
      }
    ...
    closeAddwindow() {
      ...
      this.Comp = 'OmsForm'
      this.window_name = ''
      this.omsformScrollTo(this.scroll_top)
    },
    addList() {
      this.Comp = 'OmsForm'
