        ...
        >
          ...
          <template slot-scope="{row,$index}">
            <div v-if="item.readonly || $attrs['operate-item'].prop==='detail'" style="display: flex;justify-content: center">
              {{ item.type==='PRICE' && fen2yuan(row[item.prop]) || item.type==='PRICE_AUTOMATIC' && fen2yuan(row[item.prop]) || row[item.prop] || 0 }}
            </div>
...
export default {
    ...
    comp_price_float() {
      return function(item, row, n) {
        return Object.assign(
          { decimal: Math.log10(n), max: 9999999999 },
          item.float,
          {
            max: (item.float
              ? ((typeof item.float.max === 'string') && row[item.float.max] / n ||
                (typeof item.float.max === 'number') && (item.float.max || 9999999999) ||
                (typeof item.float.max === 'function') && (item.float.max(row) || 9999999999))
              : 9999999999)
          })
      }
    },
  ...
  watch: {
    ...
    'form.details': { // 监听表格数据
      handler(n) {
        if (n) {
          this.$listeners.settleSelected(n.some(item => (!!item.supplierSettlementNo || !!item.franchiseeSettleNo)))
          if (this.$listeners.totalChange) { // thisInvoicedAmount
            this.$listeners.totalChange(n.map(item => item.thisInvoicedAmount).reduce((curr, prev) => (curr + prev), 0))
          }
      ...
      handler(n) {
        ...
        if (n.length > 0) {
          ...
          switch (this.$attrs.select_window_name) {
            case 'type_InvioceSelectIndex': // 结算单号
              ...
              temp = n.map(item =>
                Object.assign(
                  ...
                  {
                    id: item.id, // id
                    franchiseeSettleNo: item.franchiseeSettleNo, // franchiseeSettlementNo
                    settleAmount: item.settleAmount, // 应开
                    invoicedAmountTemp: item.invoicedAmount || 0, // 已开(缓存)
                    invoicedAmount: item.invoicedAmount, // 已开
                    uninvoicedAmount: item.uninvoicedAmount, // 未开
                    thisInvoicedAmount: item.uninvoicedAmount, // 本次
                    thisInvoicedAmountShow: this.fen2yuan(item.uninvoicedAmount) // 本次show
                  })
              )
              ...
              break
            case 'type_SupplierInvioceSelectIndex': // 供应商结算单号
              temp = n.map(item =>
                Object.assign(
                  this.deepClone(this.initFormDetailsItem),
                  {
                    id: item.id, // id
                    supplierSettlementNo: item.supplierSettlementNo,
                    settlementAmount: item.settlementAmount || 0, // 应付款
                    invoicedAmountTemp: item.invoicedAmount || 0, // 已开(缓存)
                    invoicedAmount: item.invoicedAmount || 0, // 已付款
                    uninvoicedAmount: item.uninvoicedAmount || 0, // 未付款
                    thisInvoicedAmount: item.uninvoicedAmount, // 本次
                    thisInvoicedAmountShow: this.fen2yuan(item.uninvoicedAmount) // 本次show
                  })
              )
          ...
          if (this.insertIndex !== null) {
            ...
            const prop = this.form.details[this.insertIndex].franchiseeSettleNo ? 'franchiseeSettleNo' : 'supplierSettlementNo'
            this.settlendelnos.push(this.form.details[this.insertIndex][prop])
            this.form.details.splice(this.insertIndex, 1)
            this.settlendelnos = this.settlendelnos.filter(item => !this.form.details.map(it => it[prop]).includes(item))
            this.$emit('update:settlendelnos', this.settlendelnos)
            // }
          } else { // 批量添加
  ...
  methods: {
    init() {
      return new Promise(resolve => {
        ...
        if (this.$attrs['operate-item'].prop === 'detail') { // 详情
          this.form.details = this.deepClone(this.$attrs['settle-details'])
          return resolve(true)
        }
        const temp = {}
        this.ArrFilter(this.$attrs['form-data'], { addFatherProp: 'settle', 'editShow': true }).map(item => {
        ...
        if (['edit', 'insert'].includes(this.$attrs['operate-item'].prop)) { // 创建,修改
          if (this.$attrs['operate-item'].prop === 'insert') { // 创建
            ...
            const priceK = this.ArrFilter(this.$attrs['form-data'], { addFatherProp: 'settle', 'editShow': true, readonly: false, type: 'PRICE' }).map(itemK => itemK.prop)
            this.ArrFilter(this.$attrs['form-data'], { addFatherProp: 'settle', 'editShow': true }).map(item => { // 初始化数组对象数据为空
              temp[item.prop] = ''
            })
            this.form.details = this.$attrs['settle-details'].length > 0
              ? this.$attrs['settle-details'].map(itemD => {
    ...
    deleteItem(row, i, isSure = false) {
      if (this.$attrs['operate-item'].prop === 'edit') {
        if (!isSure) {
      ...
      const prop = row.franchiseeSettleNo ? 'franchiseeSettleNo' : 'supplierSettlementNo'
      this.settlendelnos.push(row[prop])
      this.settlendelnos = this.settlendelnos.filter(item => !this.form.details.map(it => it[prop]).includes(item))
      this.$emit('update:settlendelnos', this.settlendelnos)
    },
    opFunc(e, i = null, row = { id: null }) {
      ...
      if (temp.supplier !== undefined) { // 供应商
        temp.supplierName = this.$attrs.invoice_form_data.supplier
        temp.supplierId = this.$attrs.invoice_form_data.supplierId
