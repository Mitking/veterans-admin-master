  ...
  <div class="heightParent" :style="{height:(windowHeight-167)+'px'}">
    ...
    <div ref="omsform" class="formFeather">
        ...
        <el-collapse :value="collapseNames" @change="collapseValue = $event[0]">
          <el-collapse-item name="1">
            ...
            <oms-invoice-form-item
              ...
              @formDataChange="invoiceFormData=$event"
              v-on="$listeners"
            />
      ...
      <el-card class="formCard">
        <oms-invoice-table ref="settleTable" :settlendelnos.sync="settleDeleteNos" :invoice_form_data="invoiceFormData" v-bind="$attrs" @dataIsChange="dataIsChange=$event" v-on="$listeners" />
      </el-card>
      <!--   发票数据     -->
      <el-card v-if="$attrs.details" class="formCard">
        <oms-table ref="Table" v-bind="$attrs" @dataIsChange="dataIsChange=$event" v-on="$listeners" />
      </el-card>
    </div>
    <!--    底部    -->
    <div class="footer">
...
<script>
import OmsInvoiceFormItem from '@/views/oms/components/OmsInvoiceFormItem'
import OmsDetailItem from '@/views/oms/components/OmsDetailItem'
import OmsTable from '@/views/oms/components/OmsTable'
import OmsInvoiceTable from '@/views/oms/components/OmsInvoiceTable'
export default {
  name: 'OmsForm',
  ...
  methods: {
    selectChange(e, v) {
      if (v === 'settleOrgId') {
        this.$refs.settleTable.settle_org_id = e
        // this.settleOrgId = e
      }
    },
    onSubmit(arg) {
      ...
      Promise.all(promise).then(res => {
        ...
        if (this.$refs.Table && this.$refs.settleTable) {
          ...
          this.$set(temp, 'Settle', this.deepClone(this.$refs.settleTable.form.details))
          this.$set(temp, 'settleDeleteNos', this.settleDeleteNos.length ? this.settleDeleteNos : []) // supplierSettlementDeleteNos
    ...
    delItem(item) {
      ...
      return new Promise((resolve) => {
        Promise.all(promise).then(res => {
          console.log('通过检验')
          const temp = {}
          this.$set(temp, 'invoicingReq', this.deepClone(this.$refs.Form.form))
          this.$set(temp, 'detailReqs', this.$refs.Table.form.details)
          ...
          if (temp.invoicingReq.invoicingTotalAmount !== (total_temp - item.total)) {
            ...
            temp.detailReqs.map(itemM => {
              if (itemM.id === '' || typeof itemM.id === 'undefined') {
                itemM.id = null
                itemM.invoicingCode = temp.invoicingReq.invoicingCode
              }
            })
    ...
    reset() {
      ...
      if (this.$refs.Table) {
        ...
        this.$refs.settleTable.init().then(() => {
          this.$refs.settleTable.$refs.Form.resetFields()
          this.$nextTick(() => {
            this.dataIsChange = false
          })
        })
