  ...
  <div>
    ...
    <el-form ref="form" class="form-fa" :rules="rules" :model="form" label-width="100px">
      ...
      >
        ...
        <el-input v-if="item.type==='number'" v-model.number="form[item.prop]" :placeholder="item.placeholder" :readonly="item.readonly" />
        ...
        <div v-if="item.type==='selectState'">{{ MenuGetPropName(item.selectData,form[item.prop],item.selectOption?item.selectOption:{ value: '', label: '' }) }}</div>
        <el-select v-if="item.type==='select' && !item.readonly" v-model="form[item.prop]" class="item-select-fa" placeholder="请选择" style="width:100%">
          <el-option
        ...
        <el-input
          ...
          :readonly="true"
          @focus="$listeners.openSelectWindow({name:item.windowName,formProData:item.formProData || {},curr:{id:null},type:'form',addedList:[]})"
        />
        <!--  价格  -->
        <el-input
        ...
        <div v-if="item.type==='PRICE_AUTOMATIC'" :style="{'width': item.formItemWidth?(item.formItemWidth+'px'):'100%'}">
          ￥ {{ comp_price_auto(item) || '0.00' }}
        </div>
        <!--  自动计算：普通  -->
        <!-- <el-input
          ...
          :placeholder="item.placeholder || '系统计算'"
        /> -->
        <div v-if="item.type==='NUMBER_AUTOMATIC'" :style="{'width': item.formItemWidth?(item.formItemWidth+'px'):'100%'}">
          {{ comp_number_auto(item) }}
        </div>
        <el-date-picker
          v-if="item.type==='date'"
...
export default {
  ...
  computed: {
    ...
    formItemShow() { // 父级数据联动渲染
      return function(e) {
        if (e.prop === 'id' || e.tableHidden === true) {
          return false
        }
  ...
  watch: {
    ...
    'form': {
      handler(n) {
        this.$listeners.baseFormChange(n)
    ...
    '$attrs.total': {
      handler(n) {
        if (typeof n === 'number') {
          this.form.invoicingTotalAmount = n
        }
      },
      immediate: true
    }
        ...
        if (['edits', 'insert'].includes(this.$attrs['operate-item'].prop)) { // 创建,批量修改
          ...
          ArrFilter(this.$attrs['form-data'], { addFatherProp: 'Req', 'editShow': true }).map(item => {
            ...
            if (item.prop === 'customerId') {
              temp.customerName = this.$attrs['current-row'].customerName
            }
            if (item.type === 'PRICE') {
