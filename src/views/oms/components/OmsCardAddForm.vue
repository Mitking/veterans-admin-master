<template>
  <div class="heightParent" :style="{height:(windowHeight-167)+'px'}">
    ...
    <div ref="cardaddform" class="formFeather">
      <!--   基础信息   -->
      <el-card class="formCard my-class my-class-line">
        <el-collapse :value="collapseNames" @change="collapseValue = $event[0]">
          <el-collapse-item name="1">
            ...
            <oms-form-item v-if="$attrs['operate-item'].prop!=='detail'" ref="Form" class="my-class-edit" v-bind="$attrs" @dataIsChange="dataIsChange=$event" @baseFormChange="baseForm=$event" v-on="$listeners" />
      ...
      <el-card v-if="$attrs.details" class="formCard">
        <oms-table ref="Table" :base_form="baseForm" v-bind="$attrs" @dataIsChange="dataIsChange=$event" v-on="$listeners" />
      </el-card>
    </div>
    <!--    底部    -->
    <div class="footer">
...
import OmsTable from '@/views/oms/components/OmsCardAddTable'
export default {
  name: 'OmsCardAddForm',
  ...
  methods: {
    onSubmit(arg) {
      ...
      Promise.all(promise).then(res => {
        ...
        if (this.$refs.Table) {
          this.$set(temp, 'detailReqs', this.deepClone(this.$refs.Table.form.details))
          this.$set(temp, 'deleteDetailIds', this.deepClone(this.$refs.Table.deleteDetails))
