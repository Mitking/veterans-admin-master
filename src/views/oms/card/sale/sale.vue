  ...
  <div>
    ...
    <el-dialog
      ...
      :title="mixin_dialog_item.selectForm==='SupplierIndex' && '供应商' ||
        mixin_dialog_item.selectForm==='Customer' && '客户列表' ||
...
import { getPage, reviews, deleteOrder } from '@/api/oms/card/sale'
export default {
  name: 'Order',
  components: { User, Customer },
  // 全局常用字段
  mixins: [MixinGlobal],
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '销售单号', prop: 'cardSaleNo', type: 'text', width: '300px' },
        { title: '销售日期', prop: 'saleDate', type: 'date', width: '300px' },
        ...
        { title: '起始卡面号', prop: 'originCardNo', type: 'text', width: '300px' },
        { title: '结束卡面号', prop: 'endCardNo', type: 'text', width: '300px' },
        { title: '客户', prop: 'customerName', reallyProp: 'customerId', type: 'selectFromPage', formProData: { typeName: '礼品卡' }, selectForm: 'Customer', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:cardSaleDetails:page'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:cardSale:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:cardSale:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:cardSale:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:cardSale:delete'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviewsFn, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:cardSale:update:review'], width: 80 }
      ],
      ids: [],
