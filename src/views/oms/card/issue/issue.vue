...
import { getPage, deleteOrder, reviews } from '@/api/oms/card/issue'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '发卡单号', prop: 'cardIssueNo', type: 'text', width: '300px' },
        { title: '下发日期', prop: 'issueDate', type: 'date', width: '300px' },
        ...
        { title: '领取人', prop: 'operatorName', reallyProp: 'recipientsId', type: 'selectFromPage', selectForm: 'User', width: '300px' }
      ],
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:cardIssueDetail:review'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:cardIssue:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:cardIssue:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:cardIssue:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:cardIssue:delete'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviewsFn, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:cardIssue:review'], width: 80 }
      ],
      ids: [],
      cardIssueNos: [],
  ...
  methods: {
    handleSelectionChange(rows) {
      ...
      this.cardIssueNos = rows.map(item => item.cardIssueNo)
