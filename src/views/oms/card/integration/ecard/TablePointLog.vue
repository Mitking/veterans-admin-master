<template>
  <div class="formFeather" :style="{height:(windowHeight-150)+'px'}">
    <el-card style="margin:24px">
      <div style="color:#42b983;font-size: 1.2rem;font-weight:500">积分流水</div>
      <el-form size="mini" label-width="80px" inline style="margin-top:24px">
        ...
        <el-form-item label="类型">
          <el-select v-model="form.changeType">
            ...
            />
            <el-option
              v-for="(itemSon,indexSon) in enumList.ecard_log_point_type"
              :key="indexSon+100"
              :label="itemSon.name"
              :value="itemSon.value"
            />
      ...
      >
        ...
        >
          <template slot-scope="{row}">
            {{ MenuGetPropName('ecard_log_point_type',row.changeType,{ value: '', label: '' }) }}
          </template>
        </el-table-column>
        <el-table-column
          prop="fid"
