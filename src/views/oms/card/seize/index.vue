<template>
  <div>
    ...
    >
      <SelectByOrder v-if="dialog_component==='button_adds'" ref="refSelect" :single="true" child="true" @selectedRows="selectItem=$event" />
      <SelectByList v-if="dialog_component==='cardNo'" ref="refSelect" :selectlist="cardNoList" :single="selectByListSingle" child="true" @selectedRows="selectItem=$event" />
  ...
  data() {
    return {
      detail_table_buttons: [
        ...
        { title: '批量收缴', show: true, prop: 'addListSeize', formProData: { orgId: '', storeId: '' }}
      ],
      form_bottons: [
        { title: '审核', mix_comp_buttons_show: this.mix_comp_buttons_show, editShowArgs: { auditStatus: '1' }, detailShowArgs: { auditStatus: '1' }, prop: 'review', loading: false, icon: 'el-icon-s-check', type: 'primary', permission: ['oms:cardSeize:update:review'] }
      ],
      ...
      table_data_arr: [ // 所有字段 show：在表格中显示，formShow：在详情中显示，defaultValue：默认值
        ...
        { title: '卡收缴单号', prop: 'cardSeizeNo', width: 230, show: true, type: 'text', editShow: true, detailShow: true, addFatherProp: 'Req', readonly: true },
        { title: '收缴日期', width: 150, prop: 'seizeDate', show: true, type: 'date', defaultValue: dayjs().format('YYYY-MM-DD'), editShow: true, detailShow: true, addFatherProp: 'Req',
          rule: [{ required: true, message: '下发日期不能为空', trigger: 'change' }] },
        { title: '收缴组织', width: 200, prop: 'orgId', show: true, type: 'select', defaultValue: [1, 2].includes(this.$store.getters.accountType) ? this.$store.getters.orgId : '', readonly: [1, 2].includes(this.$store.getters.accountType), selectData: 'organizes', selectOption: { label: 'orgShortName', value: 'id' }, editShow: true, detailShow: true, addFatherProp: 'Req',
          rule: [{ required: true, message: '组织不能为空', trigger: 'change' }] },
        { title: '收缴门店', width: 200, prop: 'storeId', show: true, type: 'select', defaultValue: [1].includes(this.$store.getters.accountType) ? this.$store.getters.storeId : '', readonly: [1].includes(this.$store.getters.accountType), selectData: 'stores', selectOption: { label: 'name', value: 'id' }, fatherProp: 'orgId', editShow: true, detailShow: true, addFatherProp: 'Req',
          rule: [{ required: true, message: '下发门店不能为空', trigger: 'change' }] },
        { title: '起始卡面号', prop: 'originCardNo', width: 150, show: true, type: 'number', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '结束卡面号', prop: 'endCardNo', width: 150, show: true, type: 'number', editShow: true, detailShow: true, addFatherProp: 'Req' },
        { title: '收缴数量', width: 100, prop: 'count', show: true, type: 'NUMBER_AUTOMATIC', design: this.mixin_auto_tab_length, editShow: true, detailShow: true, addFatherProp: 'Req', readonly: true },
        ...
        { title: '领取人', width: 150, prop: 'recipients', show: true, type: 'text', editShow: true, tableHidden: true, detailShow: true, addFatherProp: 'Req' },
  ...
  methods: {
    ...
    openModel(operateItem = {}) { // 添加、修改、详情
      ...
      if (['detail', 'edit'].includes(operateItem.prop)) { // 详情
        getDetails({ 'cardSeizeNo': this.currentRow.cardSeizeNo }).then(res => {
          if (res.code === 200) {
        ...
        this.details = []
        getSendNumber({ type: 'LPK_SJ' }).then(res => {
          if (res.code === 200) {
        ...
        case 'insert': // 新增
          ...
          if (data.detailReqs.length > 0) {
            data.detailReqs.map(item => {
              item.cardSeizeNo = data.Req.cardSeizeNo
            })
          }
          data.seizeReq = data.Req
    ...
    selectOk() {
      ...
      if (this.selectItem.type === 'order') { // 单据导入
        ...
        console.log('this.insertIndex', this.insertIndex)
