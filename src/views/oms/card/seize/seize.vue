...
import { getPage, reviews, deleteOrder } from '@/api/oms/card/seize'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '收缴单号', prop: 'cardSeizeNo', type: 'text', width: '300px' },
        { title: '收缴日期', prop: 'seizeDate', type: 'date', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:cardSeizeDetail:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:cardSeize:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:cardSeize:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:cardSeize:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:cardSeize:delete'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviewsFn, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:cardSeize:update:review'], width: 80 }
      ],
      ids: [],
