    ...
    >
      ...
      >
        ...
        <template slot-scope="{row}">
          <div v-if="item.prop==='cardAmount'">{{ row[item.prop]?fen2yuan(row[item.prop]):'-' }}</div>
          <div v-else-if="item.type==='if_type' || item.type==='if_enable'" :style="{color:row[item.prop]?null:'#999999'}">{{ MenuGetPropName(item.type,row[item.prop]) }}</div>
...
import { getPage, reviews, deleteOrder } from '@/api/oms/card/manage'
export default {
  name: 'Order',
  components: { User },
  // 全局常用字段
  ...
  data() {
    return {
      searchList: [ // 搜索栏项目
        { title: '创建单号', prop: 'cardCreateNo', type: 'text', width: '300px' },
        { title: '创建日期', prop: 'createDate', type: 'date', width: '300px' },
      ...
      operateTableList: [
        { title: '详情', show: true, prop: 'detail', click: this.$listeners.openModel, loading: false, icon: 'el-icon-document', disabled: 'isRow', type: 'primary', permission: ['oms:createCardDetail:getList'], width: 80 },
        { title: '修改', show: true, prop: 'edit', click: this.$listeners.openModel, loading: false, icon: 'el-icon-edit-outline', disabled: 'isRowNoRevEds', type: 'primary', permission: ['oms:createCard:update'], width: 80 }
      ],
      operateList: [ // 操作栏项目
        { title: '查询', show: true, prop: 'search', click: this.search, icon: 'el-icon-search', disabled: false, type: 'primary', permission: ['oms:createCard:getList'], width: 80 },
        { title: '新增', show: !this.$attrs.child, prop: 'insert', click: this.$listeners.openModel, loading: false, icon: 'el-icon-plus', disabled: false, type: 'primary', permission: ['oms:createCard:add'], width: 80 },
        { title: '删除', show: true, prop: 'delete', click: this.dels, loading: false, icon: 'el-icon-minus', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:createCard:delete'], width: 80 },
        { title: '审核', show: true, prop: 'review', click: this.reviewsFn, loading: false, icon: 'el-icon-s-check', disabled: 'idsNoRevEds', type: 'primary', permission: ['oms:createCard:review'], width: 80 }
      ],
      ids: [],
